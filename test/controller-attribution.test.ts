...
test("status and milestone attribution use the controller display name and generation",()=>{const store=new Store(":memory:"),started=new WorkflowIntake(store).start(issue,{actor:"test",source:"control"}),bodies:string[]=[];try{store.db.prepare("INSERT INTO repository_controller(repository_id,instance_id,generation,state) VALUES(1,?,4,'active')").run("11111111-1111-4111-8111-111111111111");assert.match(workflowStatusMarkdown(store,started.id),/\| Controller \| Factory 111111 \|/);new WorkflowGitHubPublisher(store,{syncWorkflow(_n,_l,body){bodies.push(body);},publishWorkflowComment(){},assignees(){return[];},assign(){},unassign(){}}).publish(started.id);assert.match(bodies[0],/generation:4/);}finally{store.db.close();}});

test("takeover notice is emitted once per untracked issue and generation",()=>{const store=new Store(":memory:"),keys=new Set<string>(),comments:string[]=[];const github={listManaged(){return[issue];},publishWorkflowComment(_n:number,key:string,body:string){if(keys.has(key))return;keys.add(key);comments.push(body);}} as any;try{const previous=observation(2,"22222222-2222-4222-8222-222222222222","Factory 222222"),current=observation(3,"33333333-3333-4333-8333-333333333333","Factory 333333");publishTakeoverNotices(store,github,previous,current);publishTakeoverNotices(store,github,previous,current);assert.equal(comments.length,1);assert.match(comments[0],/Factory 222222 to Factory 333333/);assert.match(comments[0],/`\/factory start`/);}finally{store.db.close();}});
