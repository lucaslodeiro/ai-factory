...
`);
  ...
  try {
    ...
    const thread=await fetch(`http://127.0.0.1:${port}/api/issues/owner-demo-7/thread`).then(result=>result.json()) as any;assert.deepEqual(thread.actions,["approve","answer"]);assert.equal(thread.operator.approver,true);assert.deepEqual([...new Set(thread.turns.map((turn:any)=>turn.kind))].sort(),["event","prompt"].sort());
