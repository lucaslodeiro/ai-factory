...
test("current main issue content reproduces the actionable audit findings",()=>{
 ...
 try {
  ...
  const delivery=h.milestones.get("delivery-waiting")??"";assert.match(delivery,/Review the pull request and merge it when ready/);assert.doesNotMatch(delivery,/https:\/\/github\.com\/owner\/demo\/pull\/7/);
  assert.match(h.statuses.get("questions")??"",/Open request \| clarification/);assert.match(h.statuses.get("spec")??"",/Open request \| spec-approval/);assert.match(h.statuses.get("tester-decision")??"",/Open request \| tactical-decision/);assert.match(h.statuses.get("delivery-waiting")??"",/Open request \| merge/);
  assert.match(h.statuses.get("delivery-paused")??"",/Review and merge the pull request/);assert.match(h.statuses.get("cancelled")??"",/Current actor \| None/);assert.match(h.statuses.get("cancelled")??"",/Resume the preserved work when ready/);
