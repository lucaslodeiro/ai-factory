...
async function lifecycle() {
 ...
 run("product-architect",result("brief",{summary:"A small read-only football dashboard",brief:"# Football dashboard\n\n## Decisions for you\nNone."}));await publish("brief");
 port.reply(2,"/factory approve v1");inbox.poll(workItemId);await publish("approved");
 run("product-architect",result("spec",{summary:"Specified the standings view",spec:"# Football dashboard\n\n## AC1\nShows current standings.",acceptanceCriteria:[{id:"AC1",description:"Shows current standings"}]}));await publish("spec");
...
test("issue content lifecycle publishes actionable human-facing messages",async()=>{
 ...
 try {
  ...
  const brief=h.milestones.get("brief")??"";assert.match(brief,/^# Brief v1 — awaiting approval/m);assert.match(brief,/^## Football dashboard/m);assert.doesNotMatch(brief,/## Acceptance criteria/);assert.match(brief,/`\/factory approve v1 \[guidance\]`/);assert.match(brief,/`\/factory answer <feedback>`/);assert.match(brief,/Feedback becomes a human decision for Architect, who proposes a new brief/);assert.doesNotMatch(brief,/use the command shown in the AI Factory status comment/);oneNextAction(brief);
  const spec=h.milestones.get("spec")??"";assert.match(spec,/^# Specification v1 — delivery started/m);assert.match(spec,/## Acceptance criteria[\s\S]*\| AC1 \| Shows current standings \|/);assert.doesNotMatch(spec,/\/factory approve/);oneNextAction(spec);
  assert.match(h.statuses.get("approved")??"",/Brief v1 is approved\. Architect is writing the specification/);assert.match(h.statuses.get("approved")??"",/Open request \| Architect is writing the specification/);assert.match(h.statuses.get("approved")??"",/Current actor \| Architect/);
  ...
  assert.match(h.statuses.get("questions")??"",/Open request \| Waiting for your answer/);assert.match(h.statuses.get("brief")??"",/Open request \| Waiting for approval of brief v1/);assert.match(h.statuses.get("tester-decision")??"",/Open request \| Architect is deciding/);assert.match(h.statuses.get("delivery-waiting")??"",/Open request \| Waiting for merge/);assert.doesNotMatch(Array.from(h.statuses.values()).join("\n"),/Open request \| (?:clarification|brief-approval|specification|spec-approval|tactical-decision|merge)/);
  ...
  assert.match(h.statuses.get("brief")??"",/Instance \|/);assert.match(h.statuses.get("brief")??"",/<sub>instance:/);assert.match(h.milestones.get("brief")??"",/<sub>instance:/);assert.match(h.milestones.get("spec")??"",/<sub>instance:/);
  assert.deepEqual(h.assignments.get("questions"),[]);assert.deepEqual(h.assignments.get("approved"),[]);assert.deepEqual(h.assignments.get("brief"),[]);assert.deepEqual(h.assignments.get("spec"),[]);assert.deepEqual(h.assignments.get("delivery-waiting"),[]);assert.deepEqual(h.assignments.get("delivery-paused"),[]);assert.deepEqual(failed.port.assignees(),[]);
  if(process.env.ISSUE_CONTENT_PRINT==="1")for(const [name,body] of [["DESIGN_WAITING_BRIEF_STATUS",h.statuses.get("brief")],["DESIGN_WAITING_BRIEF_COMMENT",h.milestones.get("brief")],["BUILD_QUEUED_SPEC_COMMENT",h.milestones.get("spec")],["DESIGN_QUEUED_TACTICAL_STATUS",h.statuses.get("architect-resolved")],["DELIVERY_WAITING_STATUS",h.statuses.get("delivery-waiting")],["DELIVERY_PAUSED_STATUS",h.statuses.get("delivery-paused")],["DESIGN_FAILED_STATUS",failed.body],["ARCHITECT_QUESTIONS_COMMENT",h.milestones.get("questions")]] as const)process.stdout.write(`\n===== ${name} =====\n${(body??"").replace(/^> $/gm,">")}\n===== END ${name} =====\n`);
