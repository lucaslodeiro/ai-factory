...
test("current main issue content reproduces the actionable audit findings",()=>{
 ...
 try {
  ...
  const transitions=h.statuses.get("delivery-paused")??"";assert.doesNotMatch(transitions,/product-architect|developer passed|qa requested|\/Running:/);assert.match(transitions,/Builder passed/);assert.match(transitions,/Tester requested an architectural decision/);assert.match(transitions,/\d{4}-\d\d-\d\d \d\d:\d\d UTC/);assert.doesNotMatch(transitions,/\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d/);
  ...
  assert.deepEqual(h.assignments.get("questions"),["owner"]);assert.deepEqual(h.assignments.get("approved"),[]);assert.deepEqual(h.assignments.get("spec"),["owner"]);assert.deepEqual(h.assignments.get("delivery-waiting"),["owner"]);assert.deepEqual(h.assignments.get("delivery-paused"),[]);assert.deepEqual(failed.port.assignees(),["owner"]);
