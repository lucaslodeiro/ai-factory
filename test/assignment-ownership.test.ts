...
test("two instances conflict safely, move ownership, preserve partial work and release terminal issues",async()=>{
 ...
 try{
  ...
  github.assigned=true;await a.syncRemote();let projection=new WorkflowProjections(aStore).get(aItem.id);assert.equal(projection.status,"PAUSED");assert.equal(github.labels.includes("factory-instance:a"),true);await a.syncRemote();projection=new WorkflowProjections(aStore).get(aItem.id);assert.equal(projection.status,"QUEUED");assert.equal(projection.attempt,1);
  github.assigned=false;await a.syncRemote();github.assigned=true;config.instanceName="b";await b.syncRemote();assert.equal(github.labels.includes("factory-instance:b"),true);config.instanceName="a";await a.syncRemote();assert.equal(new WorkflowProjections(aStore).get(aItem.id).status,"PAUSED");assert.equal((aStore.metadata<any[]>("runtime:assigned-issues")??[]).find(row=>row.issue===1)?.state,"other-instance");
  github.labels=["factory-instance:a"];await a.syncRemote();projection=new WorkflowProjections(aStore).get(aItem.id);new WorkflowProjections(aStore).transition({workItemId:aItem.id,expectedRevision:projection.revision,stage:projection.stage,status:"CANCELLED",actor:{type:"human",id:"owner"},source:{},reason:{code:"cancel",summary:"Cancelled"}});await a.syncRemote();assert.deepEqual(github.unassigned,["factory"]);assert.equal(github.labels.includes("factory-instance:a"),false);
