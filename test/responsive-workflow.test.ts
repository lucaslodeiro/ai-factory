...
test('slow remote sync does not own the local execution lane',async()=>{
 ...
 store.db.prepare("INSERT INTO work_items(id,issue_number,repo,created_at,updated_at,context,stage,status) VALUES('w',2,'owner/demo','now','now','{}','BUILD','QUEUED')").run();
 const remote={repositoryIssues:()=>new Promise<any[]>(resolve=>{release=resolve;}),issue:()=>({number:2,state:'OPEN',title:'Test',body:'',url:'https://github.com/owner/demo/issues/2'}),comments:()=>[],syncWorkflow(){},assignees:()=>[],assign(){},unassign(){}};
