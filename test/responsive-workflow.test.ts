...
test('slow remote sync does not own the local execution lane',async()=>{
 ...
 const remote={authenticatedLogin:()=>"factory",assignedIssues:()=>new Promise<any[]>(resolve=>{release=resolve;}),ensureLabel(){},issue:()=>({id:22,number:2,state:'OPEN',title:'Test',body:'',url:'https://github.com/owner/demo/issues/2'}),comments:()=>[],syncWorkflow(){},assignees:()=>[],assign(){},unassign(){}};
