...
test('unified issue list deduplicates GitHub rows and never replaces live local state',()=>{
 ...
 context.remote={repository:'owner/demo',issues:[{number:2,title:'Stale GitHub title',status:'FAILED',stage:'TEST'},{number:7,title:'Remote only',status:'WAITING',stage:'DESIGN',processedBy:'Another factory'}]};vm.runInContext('renderRemoteIssues(remote)',context);
