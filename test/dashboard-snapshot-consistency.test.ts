...
test('KPIs and controller share transitions; delayed snapshots and remote responses cannot roll them back',()=>{
 ...
 draw('RUNNING','2026-09-20T12:00:01.000Z');assert.equal(get('#active-count').textContent,1);assert.match(get('#controller-description').textContent,/1 active work item/);
 draw('PAUSED','2026-09-20T12:00:02.000Z');assert.equal(get('#active-count').textContent,0);assert.equal(get('#paused-count').textContent,1);assert.match(get('#controller-description').textContent,/0 active work items/);
 draw('RUNNING','2026-09-20T12:00:01.000Z');assert.equal(get('#paused-count').textContent,1);assert.match(get('#controller-description').textContent,/0 active work items/);
 vm.runInContext("renderController({state:'active',record:{activeWorkCount:9},issues:[]})",context);assert.match(get('#controller-description').textContent,/0 active work items/);
 draw('RUNNING','2026-09-20T12:00:03.000Z',true);assert.equal(get('#active-count').textContent,0);assert.match(get('#controller-description').textContent,/0 active work items/);
 draw('FAILED','2026-09-20T12:00:04.000Z');assert.equal(get('#failed-count').textContent,1);assert.match(get('#controller-description').textContent,/0 active work items/);
});
