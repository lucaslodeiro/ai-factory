...
test('unified issue list deduplicates GitHub rows and never replaces live local state',()=>{
 ...
 vm.runInContext("const pendingControls=new Map();let remoteIssueData=null,remoteIssueCheckedAt=null,remoteIssueError='',lastSnapshot=null;",context);vm.runInContext(source.slice(start,end),context);vm.runInContext(source.split('\n').find(line=>line.startsWith('function renderRemoteIssues('))!,context);
 ...
 context.remote={repository:'owner/demo',issues:[{number:2,title:'Stale GitHub title',status:'FAILED',stage:'TEST',ownership:'here',instances:['local']},{number:7,title:'Remote only',status:'WAITING',stage:'DESIGN',ownership:'other',instances:['other'],canWorkHere:true}]};vm.runInContext('renderRemoteIssues(remote)',context);
 assert.match(get('#items').innerHTML,/Local title/);assert.doesNotMatch(get('#items').innerHTML,/Stale GitHub title/);assert.match(get('#items').innerHTML,/REVIEW · RUNNING/);assert.match(get('#items').innerHTML,/Remote only/);assert.match(get('#items').innerHTML,/On other/);assert.match(get('#items').innerHTML,/Work here/);assert.equal((get('#items').innerHTML.match(/factoryControl\(/g)||[]).length,2);
