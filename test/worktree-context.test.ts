...
test("reviewer diff artifact is owned, private, excluded and safely removed",()=>{
 ...
 try{const workspaces=new Workspaces(),artifact=workspaces.prepareReviewerContext(repo,"work-1","main"),directory=path.dirname(artifact.path);assert.deepEqual(artifact.files,["feature.txt"]);assert.match(artifact.stat,/feature\.txt/);assert.match(fs.readFileSync(artifact.path,"utf8"),/feature\.txt/);assert.equal(fs.statSync(artifact.path).mode&0o777,0o600);assert.equal(fs.statSync(directory).mode&0o777,0o700);assert.equal(git(repo,["ls-files","--others","--exclude-standard"]),"");git(repo,["add","--all"]);assert.doesNotMatch(git(repo,["diff","--cached","--name-only"]),/factory-context/);workspaces.cleanupReviewerContext(repo,"work-1");assert.equal(fs.existsSync(directory),false);
  ...
  const large=workspaces.prepareReviewerContext(repo,"work-1","main");assert.ok(fs.statSync(large.path).size>10_000_000,"binary review diff must exceed the old buffer limit");workspaces.cleanupReviewerContext(repo,"work-1");
  ...
  try{config.gitCommand=fakeGit;assert.throws(()=>workspaces.prepareReviewerContext(repo,"work-1","main"),/simulated diff failure/);assert.equal(fs.existsSync(directory),false);}finally{config.gitCommand=oldGit;}
  fs.mkdirSync(directory);fs.writeFileSync(path.join(directory,"OWNER"),"foreign\n");assert.throws(()=>workspaces.prepareReviewerContext(repo,"work-1","main"),/foreign|owner/i);fs.rmSync(directory,{recursive:true});fs.symlinkSync(path.join(repo,"feature.txt"),directory);assert.throws(()=>workspaces.prepareReviewerContext(repo,"work-1","main"),/owned directory/);
