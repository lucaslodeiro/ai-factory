...
test('workflow refresh preserves the open prompt, draft and conversation scroll',()=>{
 ...
 const context=vm.createContext({applyQueueAvailability(){},document:{querySelector:()=>root},CSS:{escape:String},escapeHtml:String,relative:()=> '1m ago',roleName:()=> 'Architect',statusName:String,stateClass:()=>'',actionLabel:{retry:'Retry with guidance'},sendWorkflowMessage(){},threadDrafts:new Map([['w','keep this guidance']]),threadScroll:new Map([['w',140]]),expandedThreadTurns:new Set(['w:1']),threadResultViews:new Map(),threadResultContent:new Map()});vm.runInContext(source.split('\n').find(line=>line.startsWith('function promptReadableHtml('))!,context);const start=source.indexOf('function renderWorkflowThread('),end=source.indexOf('\nasync function loadWorkflowThread',start);vm.runInContext(source.slice(start,end),context);context.data={operator:{approver:true},actions:['retry'],turns:[{id:1,kind:'prompt',role:'product-architect',provider:'codex',model:'auto',available:true,executionId:'run',manifest:{sections:{Issue:20}}},{id:2,kind:'result',role:'product-architect',outcome:'questions',markdown:'# Architect questions',result:{outcome:'questions',summary:'Need input'}},{id:3,kind:'event',stage:'DESIGN',status:'FAILED',reason:'Design stopped',diagnosis:{summary:'Repeated summary',nextAction:'Repeated action'}}]};vm.runInContext("renderWorkflowThread('w',data)",context);assert.match(html,/data-thread-turn="w:1" open/);assert.match(html,/Agent input/);assert.match(html,/Architect prompt/);assert.match(html,/Human readable/);assert.match(html,/Agent JSON/);assert.match(html,/data-result-human ><h2>Architect questions<\/h2>/);assert.match(html,/data-result-json hidden/);assert.match(html,/"outcome": "questions"/);assert.match(html,/See the failure diagnosis above/);assert.doesNotMatch(html,/Repeated summary|Repeated action/);assert.equal(input.value,'keep this guidance');assert.equal(thread.scrollTop,140);const before=html;vm.runInContext("renderWorkflowThread('w',data)",context);assert.equal(html,before);assert.equal(input.value,'keep this guidance');assert.equal(thread.scrollTop,140);
});
