...
test("initial comment scan starts a historical authorized issue once",async()=>{
...
test("historical discovery skips Factory-labelled issues but accepts an unlabelled start",async()=>{
 const oldRepo=config.repo,oldApprovers=[...config.approvers];config.repo="owner/demo";config.approvers.splice(0,config.approvers.length,"owner");
 ...
 try{
  ...
  assert.equal((labelledStore.db.prepare("SELECT COUNT(*) count FROM work_items").get() as any).count,0);assert.equal(labelledGitHub.resultBodies.length,0);assert.equal((labelledStore.db.prepare("SELECT COUNT(*) count FROM events WHERE type='start.command_rejected'").get() as any).count,0);const rejected=labelledStore.db.prepare("SELECT payload FROM events WHERE type='command.rejected'").all() as Array<{payload:string}>;assert.equal(rejected.length,2);assert.ok(rejected.every(row=>JSON.parse(row.payload).reason==="already processed by Factory"));labelledStore.db.close();
...
test("a fresh start comment can reclaim a Factory-labelled issue after historical discovery",async()=>{
