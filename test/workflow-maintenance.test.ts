...
test("maintenance binds confirmation to exact revisions, resumes queued work and completes its barrier",async()=>{const f=fixture();try{const request=f.maintenance.request("update","owner");assert.equal(request.confirmationRequired,true);assert.equal(request.affected[0].title,"Safe maintenance");await f.maintenance.confirm(request.id);assert.equal(f.projections.get("w").status,"PAUSED");assert.deepEqual(f.maintenance.resume(request.id),["w"]);assert.equal(f.projections.get("w").status,"QUEUED");assert.equal(f.projections.get("w").attempt,1);assert.equal((f.store.db.prepare("SELECT status FROM maintenance_operations WHERE id=?").get(request.id) as {status:string}).status,"completed");}finally{f.store.db.close();}});
...
test("maintenance timeout leaves work paused and refuses to become ready",async()=>{const f=fixture(true);try{const request=f.maintenance.request("daemon-stop","owner"),stuck={isRunning:()=>true,interrupt:()=>true} as any,maintenance=new WorkflowMaintenance(f.store,stuck);await assert.rejects(maintenance.confirm(request.id,40),/Timed out/);assert.equal(f.projections.get("w").status,"PAUSED");assert.equal((f.store.db.prepare("SELECT status FROM maintenance_operations WHERE id=?").get(request.id) as any).status,"failed");}finally{f.store.db.close();}});
