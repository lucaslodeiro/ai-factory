...
test("rejected Architect attempts remain key updates with the validation reason",()=>{
 const ts="2026-09-22T20:05:00.000Z",row=(id:number,run_id:string,type:string,payload:object)=>({id,ts,run_id,type,payload:JSON.stringify(payload)}),message="A proposed specification must return questions: []";
 const events=[row(1,"run-1","execution.started",{role:"product-architect"}),row(2,"run-1","execution.finished",{status:"succeeded"}),row(3,"run-1","execution.invalid_result",{message}),row(4,"run-1","workflow.transition",{to:{stage:"DESIGN",status:"QUEUED"},reason:{code:"invalid-result-retry",summary:"Result rejected by the validator; retrying once with the message"}}),row(5,"run-2","execution.started",{role:"product-architect"}),row(6,"run-2","execution.finished",{status:"succeeded"})];
 const store={db:{prepare(sql:string){if(sql.startsWith("SELECT 1 FROM work_items"))return{get:()=>({})};if(sql.startsWith("SELECT id,ts,run_id,type,payload"))return{all:()=>events};if(sql.startsWith("SELECT execution_id,message,created_at"))return{all:()=>[{execution_id:"run-2",message,created_at:ts}]};if(sql.startsWith("SELECT status,started_at,finished_at"))return{get:()=>({status:"succeeded",started_at:"2026-09-22T20:00:00.000Z",finished_at:ts})};throw new Error(`Unexpected query: ${sql}`);}}} as unknown as Store;
 const turns=workflowThread(store,"w"),attempts=turns.filter(turn=>turn.kind==="execution");assert.equal(attempts.length,2);for(const turn of attempts){assert.equal(turn.status,"rejected");assert.equal(turn.milestone,true);assert.match(String(turn.reason),/Result rejected:.*questions: \[\]/);}assert.equal(turns.find(turn=>turn.kind==="event")?.milestone,true);
});
...
test("interrupt and retry preserves partial work and adds the previous attempt to the next context",async()=>{
 const previousRepo=config.repo;config.repo="owner/demo";const store=new Store(":memory:");try{
  store.db.prepare("INSERT INTO work_items(id,issue_number,repo,branch,base_branch,created_at,updated_at,context,stage,status,attempt,correction_cycles,active_run_id) VALUES('w',7,'owner/demo','factory/issue-7','main','now','now',?,'BUILD','RUNNING',1,2,'run')").run(JSON.stringify({cwd:"/work",attemptStart:{executionId:"run",head:"abc",stage:"BUILD",startedAt:"before"}}));store.db.prepare("INSERT INTO specs(work_item_id,version,body,criteria,approved_by,approved_at) VALUES('w',1,'spec','[]','owner','now')").run();store.db.prepare("INSERT INTO executions(id,work_item_id,role,stage,status,started_at) VALUES('run','w','developer','BUILD','running','now')").run();
...
test("interrupt retry rejects before changing workflow state when the process does not stop",async()=>{
 const previousRepo=config.repo;config.repo="owner/demo";const store=new Store(":memory:"),github=new ChatGitHub();try{
  store.db.prepare("INSERT INTO work_items(id,issue_number,repo,branch,base_branch,created_at,updated_at,context,stage,status,active_run_id) VALUES('w',7,'owner/demo','factory/issue-7','main','now','now','{}','BUILD','RUNNING','run')").run();store.db.prepare("INSERT INTO executions(id,work_item_id,role,stage,status,started_at) VALUES('run','w','developer','BUILD','running','now')").run();
