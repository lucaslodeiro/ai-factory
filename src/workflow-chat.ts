...
export function workflowThread(store:Store,workItemId:string):WorkflowThreadTurn[]{
 ...
 for(const turn of executions.values()){
  ...
  Object.assign(turn,{startedAt,finishedAt,durationMs:Number.isFinite(started)&&Number.isFinite(finished)&&finished>=started?finished-started:null,status:turn.status??(run?run.status==="running"&&run.finished_at?"failed":run.status:turn.result?"succeeded":finishedAt?"succeeded":"running")});
 }
 return turns;
}
