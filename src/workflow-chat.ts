...
import {budgetState,budgetWarningThresholds,liveBudget} from "./budget.js";
...
export function workflowThread(store:Store,workItemId:string):WorkflowThreadTurn[]{
 ...
 for(const row of rows){let value:any={};try{value=JSON.parse(row.payload);}catch{}
  ...
  if(row.type==="workflow.transition"){const transition={stage:value.to?.stage,status:value.to?.status,reason:workflowExecutionSummary(value.reason?.summary??"Workflow changed",value.to?.stage),actor:value.actor??null};const turn:WorkflowThreadTurn={id:row.id,at:row.ts,kind:"event",...transition,milestone:["invalid-result-retry","agent-error-retry"].includes(value.reason?.code)||["WAITING","FAILED","PAUSED","CANCELLED","COMPLETED"].includes(value.to?.status)&&value.reason?.code!=="continued"};if(value.to?.status==="FAILED"){const failure=new WorkflowFailures(store).active(workItemId);if(failure){turn.failure=workflowFailureEvidence(store,failure);turn.diagnosis=diagnoseWorkItem(store,workItemId);turn.publication=threadPublication(store,failurePublicationKey(failure.id));}}turns.push(turn);continue;}
 ...
 for(const turn of executions.values()){
  ...
  if(status==="running"&&startedAt){const progress=store.metadata<ExecutionProgress>(progressKey(String(turn.executionId))),budget=budgetState(store,workItemId);const warning=progressWarning(progress,startedAt);const usage=progress?.usageTokens??null,live=liveBudget(budget.granted,budget.consumed,usage);turn.progress={events:progress?.events??0,lastProgressAt:progress?.lastProgressAt??null,tool:progress?.tool??null,toolStartedAt:progress?.toolStartedAt??null,warning:warning?.reason??null,usageTokens:usage,budgetConsumed:live.consumed,budgetGranted:budget.granted,budgetPercent:live.percent,budgetGracePercent:config.tokenBudgetGracePercent,validationAttempts:progress?.validationAttempts??0,lastValidationError:progress?.lastValidationError??null};}
