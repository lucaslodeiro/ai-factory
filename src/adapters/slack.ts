...
import { slackPayload, type NotificationPort } from "../notifications.js";
export class SlackAdapter implements NotificationPort {
  ...
  async notify(text: string) {
    ...
    const r = await fetch(this.webhook, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(slackPayload(text)), signal: AbortSignal.timeout(10000) });
