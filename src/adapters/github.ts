...
export class GitHubAdapter implements GitHubPort {
 ...
 listManaged(): Issue[] {
  const managed = new Set([...Object.keys(statePresentation).map(state => `factory:${state.toLowerCase().replaceAll("_","-")}`),
   "factory:design","factory:build","factory:test","factory:review","factory:delivery","factory:done","factory:waiting","factory:failed","factory:paused","factory:cancelled"]);
