...
export interface TokenUsage { inputTokens:number|null; outputTokens:number|null; cachedTokens:number|null; totalTokens:number|null; }
...
function usageFromObject(value:unknown): TokenUsage | null {
 ...
 return {inputTokens:input,outputTokens:output,cachedTokens:cached,totalTokens:explicit ?? (input ?? 0)+(output ?? 0)+(cached ?? 0)};
}
...
function merge(values:TokenUsage[]) {
 ...
 return {inputTokens:sum("inputTokens"),outputTokens:sum("outputTokens"),cachedTokens:sum("cachedTokens"),totalTokens:sum("totalTokens")};
}
...
export function extractTokenUsage(provider: AgentProvider | undefined,stdout: string,stderr: string): TokenUsage | null {
 if (provider === "codex") {
  ...
  if (match) return {inputTokens:null,outputTokens:null,cachedTokens:null,totalTokens:Number(match[1].replaceAll(",",""))};
 }
