...
function humanRequestAction(store:Store,request:NonNullable<ReturnType<WorkflowRecords["activeRequest"]>>) {
 ...
 if(request.payload.type==="brief-approval")return `Read brief v${request.specVersion}: the decisions that need you, the solution and the scope. Nothing detailed is written yet. When the brief proposes a split into stories, approving also approves the split. Post one new comment.\n\n**Approve**${command(`/factory approve v${request.specVersion} [guidance]`)}\nApproving accepts every recommendation in the brief. Architect then writes the specification and its acceptance criteria; if the change has significant UX impact, you approve a prototype before the Builder starts. Optional guidance becomes a spec-scoped instruction.\n\n**Change a decision or request changes**${command("/factory answer <feedback>")}\nFeedback becomes a human decision for Architect, who proposes a new brief.`;
 if(request.payload.type==="spec-approval")return `Look at the prototype comment for SPEC v${request.specVersion}: its screenshots show the approved brief as the Designer built it. The specification is folded in its own comment; you do not need to read it. Post one new comment.\n\n**Approve**${command(`/factory approve v${request.specVersion} [guidance]`)}\nThe Builder starts from the specification and the prototype. Optional guidance becomes a spec-scoped instruction.\n\n**Request changes**${command("/factory answer <feedback>")}\nFeedback becomes a human decision for Architect, who proposes a new brief.`;
...
function nextAction(store:Store,workItemId:string) {
 ...
 if(request?.payload.kind==="request"&&request.payload.owner==="designer")return box(`Designer is preparing a prototype of the specification. Its approval command arrives with the screenshots; no human action is required yet. You can still pause or cancel the workflow.${command("/factory pause [reason]")}${command("/factory cancel [reason]")}`);
...
function requestLabel(request:ReturnType<WorkflowRecords["activeRequest"]>) {
 ...
 if(request.payload.type==="brief-approval")return `Waiting for approval of brief v${request.specVersion}`;
 if(request.payload.type==="specification")return "Architect is writing the specification";
 if(request.payload.type==="spec-approval")return `Waiting for approval of the prototype for SPEC v${request.specVersion}`;
...
export function workflowStatusMarkdown(store:Store,workItemId:string) {
 ...
 let latestSummary="";if(latest)try{const payload=JSON.parse(latest.payload) as {role:"product-architect"|"designer"|"developer"|"qa"|"reviewer";result:{summary:string}},summary=clipSummary(publishedText(payload.result.summary)),heading=projection.status==="FAILED"?"Last agent report":"Latest delivery summary";latestSummary=`\n\n### ${heading}\n\n**${roleShortName(payload.role)}:** ${summary.text}${summary.clipped?`\n\nSummary clipped. Open execution \`${latest.run_id}\` in the dashboard for the full result.`:""}`;}catch{}
