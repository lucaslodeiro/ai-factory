...
export function workflowStatusMarkdown(store:Store,workItemId:string) {
 ...
 let latestSummary="";if(latest)try{const payload=JSON.parse(latest.payload) as {role:"product-architect"|"developer"|"qa"|"reviewer";result:{summary:string}},summary=clipSummary(payload.result.summary),heading=projection.status==="FAILED"?"Last agent report":"Latest delivery summary";latestSummary=`\n\n### ${heading}\n\n**${roleShortName(payload.role)}:** ${summary.text}${summary.clipped?`\n\nSummary clipped. Open execution \`${latest.run_id}\` in the dashboard for the full result.`:""}`;}catch{}
