...
export function notificationText(w: WorkItem, detail?: string) {
  ...
  switch (w.state) {
    ...
    case "PAUSED": heading = "⏸️ Work paused"; content = [`The workflow paused during *${stateLabels[w.context.resume ?? "SPEC"]}*. Post ${command("/factory retry")} as a new issue comment or use Retry in the dashboard when ready.`]; break;
    ...
    case "CANCELLED": heading = "⏹️ Work cancelled"; content = [`The workflow was cancelled during *${stateLabels[w.context.resume ?? "SPEC"]}*. Post ${command("/factory retry")} as a new issue comment or use Retry in the dashboard to resume it.`]; break;
  }
