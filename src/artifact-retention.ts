...
import {progressKey} from "./execution-progress.js";
...
export function pruneExecutionArtifacts(store:Store,now=Date.now()) {
 ...
 for(const row of rows){const directory=path.join(config.dataDir,"runs",row.id);for(const name of removable){const file=path.join(directory,name);if(fs.existsSync(file)){fs.rmSync(file,{force:true});files++;}}store.db.prepare("DELETE FROM metadata WHERE key=?").run(progressKey(row.id));}
