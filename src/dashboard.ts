...
import {availableMessageActions,promptArtifact,prototypeScreenshot,workflowThread,workflowContinuations,type MessageAction,statusPublication} from "./workflow-chat.js";
...
function eventPresentation(type: string, payload: string) {
  try {
    ...
    if (type === "control.applied") {
      ...
      if (value.kind === "continue-issue") return { title:`Issue #${value.result?.issue ?? "?"} continued`,details:`This installation adopted the state carried by the issue and preserved it in Paused state.`,severity:"success",category:"Control" };
...
function remoteIssuesView(store:Store,settingsRoot:string){
 ...
 const runtime=new Map((store.metadata<Array<{issue:number;state:string;sourceInstance?:string}>>("runtime:assigned-issues")??[]).map(row=>[row.issue,row]));
 const own=`factory-instance:${config.instanceName}`;const issues=github.assignedIssues(github.authenticatedLogin()).map(issue=>{const labels=(issue.labels??[]).map(label=>label.name),instances=labels.filter(label=>label.startsWith("factory-instance:")),stage=labels.find(label=>["factory:design","factory:build","factory:test","factory:review","factory:delivery","factory:done"].includes(label)),status=labels.find(label=>["factory:waiting","factory:failed","factory:paused","factory:cancelled"].includes(label)),trackedHere=tracked.has(issue.id),ownership=trackedHere||instances.length===1&&instances[0]===own?"here":instances.includes(own)?"conflict":instances.length?"other":"unclaimed",continuation=runtime.get(issue.number);return{id:issue.id,number:issue.number,title:issue.title,url:issue.url,stage:stage?.slice(8)??null,status:status?.slice(8)??(stage?"active":null),trackedHere,ownership,instances:instances.map(label=>label.slice("factory-instance:".length)),canWorkHere:ownership!=="here",continuationState:continuation?.state??null,sourceInstance:continuation?.sourceInstance??null,canContinue:continuation?.state==="continuation-waiting"};});
...
export function createDashboardServer(store: Store, settingsRoot = process.cwd()) {
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if(req.method==="GET"&&/^\/api\/issues\/[^/]+\/thread$/.test(url.pathname)){try{const id=decodeURIComponent(url.pathname.split("/")[3]);return json(res,200,{workItemId:id,operator:dashboardOperator(store),actions:availableMessageActions(store,id),publication:statusPublication(store,id),sync:{...(store.metadata<Record<string,unknown>>("runtime:github-sync")??{}),daemonRunning:daemonState(store).running},continuations:workflowContinuations(store,id),turns:workflowThread(store,id)});}catch(error){return json(res,(error as {statusCode?:number}).statusCode??500,{error:(error as Error).message});}}
      ...
      if(req.method==="GET"&&url.pathname.match(/^\/api\/executions\/[^/]+\/prototype$/)){const shot=prototypeScreenshot(store,decodeURIComponent(url.pathname.split("/")[3]),url.searchParams.get("path")??"");if(!shot)return json(res,404,{error:"Unknown prototype screenshot"});res.writeHead(200,{"content-type":shot.type,"cache-control":"private, max-age=3600","x-content-type-options":"nosniff"});return res.end(shot.bytes);}
      ...
      if (req.method === "POST" && url.pathname === "/api/control") {
        const body = await readBody(req) as { kind?: string; target?: string;text?:string;action?:MessageAction };
