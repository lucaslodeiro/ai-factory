...
export const statePresentation: Record<WorkState, { title: string; color: string; description: string }> = {
 ...
 SPEC: { title: "Design", color: "5319e7", description: "Architect is preparing the work" },
 WAITING_HUMAN: { title: "Waiting for your response", color: "fbca04", description: "Human approval, clarification or guidance required" },
 DEVELOPMENT: { title: "Build", color: "1d76db", description: "Builder is implementing the approved specification" },
 QA: { title: "Test", color: "0e8a16", description: "Tester is verifying the acceptance criteria" },
 REVIEW: { title: "Review", color: "006b75", description: "Reviewer is inspecting implementation and evidence" },
...
const stageName = (state?: WorkState) => state ? stateName(state) : "Design";
...
export function reportMarkdown(role: AgentRole, version: number, r: AgentResult, pr?: string) {
 ...
 const sections = [`## ${roleShortName(role)} — ${outcomes[r.outcome]}`, `SPEC v${version}`, clip(r.summary, 1800)];
 ...
 const action = pr ? nextAction(`Review [the pull request](${pr}) and merge it when satisfied. The factory never merges automatically.`)
  ...
  : r.outcome === "decision" ? noAction("Architect will evaluate the decision and route the approved workflow without changing the specification silently.")
  : role === "developer" ? noAction("The factory will run Tester next.")
  : role === "qa" ? noAction("The factory will run Reviewer next.")
...
export function progressMarkdown(w: WorkItem) {
 ...
 const waiting = w.state === "WAITING_HUMAN" ? c.waiting === "approval"
  ...
  : c.waiting === "loop"
   ? `### Why the factory is waiting\n\n**The automatic correction limit was reached.** The current work and reports are preserved, but Architect needs your guidance before replanning.`
   : `### Why the factory is waiting\n\n**Architect needs clarification.** The current draft is preserved and work will resume after an authorized answer is posted.` : "";
 const action = w.state === "WAITING_HUMAN"
  ? c.waiting === "approval"
   ? nextAction(`Choose one option and post it as a **new comment**.\n\n**Approve SPEC v${c.version}**\n\n${commandBox(`/factory approve v${c.version}`)}\n\n**Request changes or add context**\n\n${commandBox("/factory answer <feedback>")}`,true)
   : c.waiting === "loop"
    ? nextAction(`Tell Architect how to proceed by posting a new comment:\n\n${commandBox("/factory answer <guidance>")}`)
    : nextAction(`Answer Architect in a new comment. You can write multiple lines after the command:\n\n${commandBox("/factory answer\n<your response>")}`)
 ...
 return `## AI Factory — ${statePresentation[w.state].title}\n\n${statePresentation[w.state].description}.\n\n${table(["Milestone", "Status"], [
  ...
  ["Build · Builder", stage("developer")], ["Test · Tester", stage("qa")], ["Review · Reviewer", stage("reviewer")],
...
export function architecturalReviewMarkdown() {
 return `## Additional architectural review\n\nArchitect detected high complexity or risk. A strong-profile architectural review will refine the draft before any specification is published for approval.\n\n${noAction("The factory will publish the reviewed specification or request clarification next.")}`;
}
export function correctionLimitMarkdown() {
 return `## Automatic correction limit reached\n\nThe current implementation, reports and worktree are preserved. Architect needs human guidance before replanning the next attempt.\n\n${nextAction(`Post a new comment describing how to proceed:\n\n${commandBox("/factory answer <guidance>")}`)}`;
}
