...
// Retry one transient provider failure; authentication, permissions, invalid configuration,
// timeouts and deliberate interruptions need a person or a changed environment instead.
export function recoverableAgentError(error:unknown){return error instanceof InvalidResultError||error instanceof Error&&/(?:rate.?limit|overload(?:ed)?|temporarily unavailable|ECONNRESET|ETIMEDOUT|connection reset)/i.test(error.message);}
...
export function rejectionContinuation(message:string){return `# Correct your result\n\nThe factory rejected the result you returned above: ${message}\n\nThe work itself is done; do not redo it or explore again. Return a corrected result for the same work that fixes only what the rejection names, under the same result contract.`;}
...
// The Builder, the Tester or the Designer coming back to work it already did: after a finding, a
// change requested on the pull request, or feedback on a prototype. Continuing its own session keeps
// what it read and why it made what it made; only what happened since is sent.
function returnContinuation(role:"developer"|"qa"|"designer"){
 if(role==="developer")return `# Continue: the work came back\n\nThe work you delivered above came back with findings to fix, from verification, from review or from the human reviewing the pull request; they are under "Open findings required by this role" below. Your contract, the approved specification and the repository you read are all above in this session. The branch has changed since your run: other roles committed to it, so re-read a file before relying on what you remember of it, and do not explore again what has not changed. Fix what the findings name, re-run the checks that cover it, and return under the same result contract. The sections below are the current state of the work item.`;
 if(role==="qa")return `# Continue: verify the changes\n\nThe Builder changed the branch since your last verification, to address findings; what changed is under "Changed files" below. Your contract, the approved specification and the tests you wrote are all above in this session. Re-read the changed files and anything they touch before relying on what you remember, re-run your verification against the new head, and do not repeat checks the change cannot affect. Return under the same result contract. The sections below are the current state of the work item.`;
 return `# Continue: revise your prototype\n\nThe human reviewed the prototype you made above and asked for changes: the feedback is under "Active human decisions" below, and the specification below is the current one, which the Architect may have revised for it. Your contract and the prototype files you wrote are above in this session and still on disk. Change what the feedback and the specification require, keep what they do not touch, retake the screenshots that changed, and return under the same result contract. The sections below are the current state of the work item.`;
}
...
const stopReasons:Record<string,string>={
 ...
 "provider-stalled":"Your provider stopped responding for five minutes while no tool was running, so the Factory stopped the run. If you had already finished the work, return the result now without redoing anything; otherwise continue where you were",
};
...
export class WorkflowRunner {
 ...
 async run(workItemId:string) {
  ...
  let started:ReturnType<WorkflowScheduler["begin"]>|undefined,preparing=true,resumed:string|undefined;
  ...
  reviewerContext=role==="reviewer"?this.workspaces.prepareReviewerContext(cwd,workItemId,base):undefined;
   ...
   // Every role keeps its session too, so a result the validator rejects is corrected by continuing
   // the run that produced it: the turn completed, the work is done, only the report was wrong.
   const sessions=resumableProviders.has(selection.provider);
   ...
   // The Builder and the Tester continue their last applied run under the same spec, whatever sent the
   // work back; the Designer continues its last prototype, which feedback may have made the spec revise.
   const deliverySession=!rejectedSession&&!stopped&&sessions&&!route&&(role==="developer"||role==="qa"||role==="designer")?this.roleSession(workItemId,role,role==="designer"?undefined:specVersion,selection,currentContext.unresumableSession):undefined;
  ...
  const instructions=correcting?`${continuation}${runtimeNote}`:`${resumed?continuation:contract}\n\n${assembled.markdown}${runtimeNote}`,before=baseline?.head??this.workspaces.head(cwd);
   ...
   if(role==="qa"&&config.verifyCommand){
    ...
    if(verification.exitCode!==0&&result.outcome==="pass")result={...result,outcome:"changes",findings:[...result.findings,{classification:"auto-fix",severity:"major" as const,evidence:`Factory verification failed: \`${verification.command}\` exited ${verification.exitCode}.\n\n${verification.outputTail}`}]};
   }
  ...
  } catch(error){if(!started){if(!preparing)throw error;this.scheduler.rejectQueued(workItemId,new Error(`Could not prepare workflow execution: ${error instanceof Error?error.message:String(error)}`),error instanceof InvalidContextError?"invalid-context":error instanceof SyncConflictError?"integration":"execution");return true;}const interrupted=this.store.db.prepare("SELECT status,interruption_reason FROM executions WHERE id=?").get(started.executionId) as {status:string;interruption_reason:string|null}|undefined;if(interrupted?.status==="interrupted"&&interrupted.interruption_reason==="interrupted-for-guidance"){this.store.event("execution.discarded",{executionId:started.executionId,reason:"Human interrupted the attempt with new guidance"},workItemId,started.executionId);return true;}if(resumed&&interrupted?.status==="failed"){this.store.event("session.unavailable",{sessionId:resumed,spentNothing:this.settleRefusedResume(started.executionId),error:sanitizeFailureEvidence(error instanceof Error?error.message:String(error),600)},workItemId,started.executionId);this.updateContext(workItemId,{unresumableSession:resumed});}const stalled=interrupted?.status==="interrupted"&&interrupted.interruption_reason==="provider-stalled";if(error instanceof InvalidResultError||stalled||interrupted?.status==="failed"&&(recoverableAgentError(error)||Boolean(resumed))){const kind=error instanceof InvalidResultError?"invalid-result":"transient-error";const current=new WorkflowProjections(this.store).get(workItemId),row=this.store.db.prepare("SELECT context FROM work_items WHERE id=?").get(workItemId) as {context:string},context=JSON.parse(row.context||"{}") as {invalidResultRetry?:{stage?:DeliveryStage;attempt?:number;count?:number}};const previous=context.invalidResultRetry?.stage===current.stage&&context.invalidResultRetry.attempt===current.attempt?context.invalidResultRetry:undefined,retryCount=previous?.count??0;if(current.status==="RUNNING"&&current.activeRunId===started.executionId&&retryCount<config.recoverableErrorRetries){const message=stalled?"The provider stopped responding for five minutes while no tool was running":sanitizeFailureEvidence(error instanceof Error?error.message:String(error),1600);this.store.event(kind==="invalid-result"?"execution.invalid_result":"execution.retryable_error",{message},workItemId,started.executionId);this.updateContext(workItemId,{invalidResultRetry:{stage:current.stage,attempt:current.attempt,executionId:started.executionId,message,kind,count:retryCount+1}});new WorkflowProjections(this.store).transition({workItemId,expectedRevision:current.revision,stage:current.stage,status:"QUEUED",actor:{type:"orchestrator",id:"runner"},source:{executionId:started.executionId},reason:{code:kind==="invalid-result"?"invalid-result-retry":"agent-error-retry",summary:kind==="invalid-result"?"Result rejected by the validator; retrying with the message":"Temporary agent error; retrying with the message"}});return true;}}this.store.event("workflow.result_failed",{error:error instanceof Error?error.message:String(error)},workItemId,started.executionId);const summary=executionOutcomeText(interrupted?.status,interrupted?.interruption_reason??undefined,started.role);const progress=interrupted?.status==="timed_out"?this.store.metadata<ExecutionProgress>(progressKey(started.executionId)):undefined;const evidence=progress?` Last observable activity: ${progress.lastProgressAt??"none"}; provider events: ${progress.events}; ${progress.tool?`${progress.tool} was still running`:`last tool: ${progress.lastTool??"unknown"}`}. This does not establish whether the agent was blocked.`:"";this.scheduler.fail(workItemId,started.executionId,summary?new Error(summary+evidence):error,error instanceof InvalidContextError?"invalid-context":error instanceof InvalidResultError?"invalid-result":"execution");return true;}
  finally{
 ...
 // A resume the provider refused before reaching the model spent nothing. Left unmeasured, it would
 // hold the automatic fresh retry until a person acknowledged a run that cost no tokens.
 private settleRefusedResume(executionId:string){
  const row=this.store.db.prepare("SELECT payload FROM events WHERE run_id=? AND type='execution.finished' ORDER BY id DESC LIMIT 1").get(executionId) as {payload:string}|undefined;
  const finished=row?JSON.parse(row.payload) as {usage?:unknown;activity?:{eventTypes?:Record<string,number>}|null}:{},types=finished.activity?.eventTypes??{};
 ...
 private roleSession(workItemId:string,role:AgentRole,specVersion:number|undefined,selection:ModelSelection,unresumable?:string){
  const latest=this.store.db.prepare("SELECT e.run_id FROM events e JOIN events s ON s.run_id=e.run_id AND s.type='model.selected' WHERE e.work_item_id=? AND e.type='agent.result' AND json_extract(e.payload,'$.role')=? AND (? IS NULL OR json_extract(s.payload,'$.specVersion')=?) ORDER BY e.id DESC LIMIT 1").get(workItemId,role,specVersion??null,specVersion??null) as {run_id:string}|undefined;
  return latest?this.runSession(latest.run_id,selection,unresumable):undefined;
 }
 // The session a run kept, when the same provider and model can continue it.
 private runSession(executionId:string,selection:ModelSelection,unresumable?:string){
  const event=(type:string)=>{const row=this.store.db.prepare("SELECT payload FROM events WHERE run_id=? AND type=? ORDER BY id DESC LIMIT 1").get(executionId,type) as {payload:string}|undefined;return row?JSON.parse(row.payload) as Record<string,unknown>:undefined;};
  const sessionId=event("execution.finished")?.sessionId,selected=event("model.selected"),chosen=selected?.selection as ModelSelection|undefined;
 ...
 private prBody(workItemId:string,issueNumber:number,result:Pick<AgentResult,"summary">){
  ...
  if(qa)sections.push(resultMarkdown("qa",{brief:"",spec:"",acceptanceCriteria:[],stories:[],testCandidates:[],questions:[],taskAssessment:null,dependencies:[],nextRole:null,reviewChecks:[],...qa},spec.version,undefined,{reportOnly:true}));
