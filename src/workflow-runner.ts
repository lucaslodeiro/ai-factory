...
export class WorkflowRunner {
 ...
 async run(workItemId:string) {
  ...
  const retrySummary=role==="developer"&&projection.attempt>1?this.workspaces.changeSummary(cwd):undefined;
  reviewerContext=role==="reviewer"?this.workspaces.prepareReviewerContext(cwd,workItemId):undefined;
   ...
   const currentContext=JSON.parse((this.store.db.prepare("SELECT context FROM work_items WHERE id=?").get(workItemId) as {context:string}).context||"{}") as {previousAttempt?:unknown};
   const assembled=this.assembler.assemble({workItemId,role,specVersion,budgetBytes:budget.bytes-contractBytes-2,budgetSource:budget.source,issue:{title:context.title??`Issue #${row.issue_number}`,body:context.body??""},changedFiles:(reviewerContext??summary??retrySummary)?.files,diffStat:(reviewerContext??summary??retrySummary)?.stat,diffPath:reviewerContext?.path,qaEvidence:role==="reviewer"?this.latestResult(workItemId,"qa"):undefined,previousAttempt:currentContext.previousAttempt});
  ...
  const instructions=`${contract}\n\n${assembled.markdown}`,before=baseline?.head??this.workspaces.head(cwd);
   ...
   this.updateContext(workItemId,{attemptStart:{executionId:started.executionId,head:before,startedAt:new Date().toISOString(),stage:projection.stage}});
 ...
 recordPreviousAttempt(workItemId:string,executionId:string,interruptedAt:string,reason:string,guidanceRecordId?:string){const row=this.store.db.prepare("SELECT context,stage FROM work_items WHERE id=?").get(workItemId) as {context:string;stage:DeliveryStage},context=JSON.parse(row.context||"{}") as {cwd?:string;attemptStart?:{executionId?:string;head?:string;startedAt?:string;stage?:string}};if(!context.cwd)return;const baseline=context.attemptStart?.executionId===executionId?context.attemptStart.head:undefined,summary=baseline&&this.workspaces.changeSummarySince?this.workspaces.changeSummarySince(context.cwd,baseline):this.workspaces.changeSummary(context.cwd),role=({DESIGN:"product-architect",BUILD:"developer",TEST:"qa",REVIEW:"reviewer",DELIVERY:"reviewer"} as Record<string,AgentRole>)[context.attemptStart?.stage??row.stage],lastResult=this.latestResult(workItemId,role);this.updateContext(workItemId,{previousAttempt:{interruptedAt,reason,startingCommit:baseline??null,files:summary.files,diffStat:summary.stat,lastResult:lastResult??null,guidanceRecordId:guidanceRecordId??null}});}
