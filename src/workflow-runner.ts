...
// A correction cycle brings the Builder or the Tester back to work it already did. Continuing its own
// session keeps what it read and why it wrote what it wrote; only what happened since is sent.
...
export class WorkflowRunner {
 ...
 private roleSession(workItemId:string,role:AgentRole,specVersion:number,selection:ModelSelection,unresumable?:string){
  const latest=this.store.db.prepare("SELECT e.run_id FROM events e JOIN events s ON s.run_id=e.run_id AND s.type='model.selected' WHERE e.work_item_id=? AND e.type='agent.result' AND json_extract(e.payload,'$.role')=? AND json_extract(s.payload,'$.specVersion')=? ORDER BY e.id DESC LIMIT 1").get(workItemId,role,specVersion) as {run_id:string}|undefined;
  return latest?this.runSession(latest.run_id,selection,unresumable):undefined;
 }
