...
export class WorkflowRunner {
 ...
 async run(workItemId:string) {
  ...
  reviewerContext=role==="reviewer"?this.workspaces.prepareReviewerContext(cwd,workItemId,base):undefined;
   ...
   const currentContext=JSON.parse((this.store.db.prepare("SELECT context FROM work_items WHERE id=?").get(workItemId) as {context:string}).context||"{}") as {previousAttempt?:{stage?:DeliveryStage;attempt?:number};invalidResultRetry?:{stage?:DeliveryStage;attempt?:number;executionId?:string;message?:string;kind?:"invalid-result"|"transient-error"}};
