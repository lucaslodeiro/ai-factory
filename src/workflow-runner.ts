...
export class WorkflowRunner {
 ...
 async run(workItemId:string) {
  ...
  reviewerContext=role==="reviewer"?this.workspaces.prepareReviewerContext(cwd,workItemId,base):undefined;
   ...
   resumed=rejectedSession??(sessions&&role==="product-architect"?this.architectSession(workItemId,selection,currentContext.unresumableSession):undefined);
   const continuation=!resumed?"":correcting?rejectionContinuation(invalidResultRetry!.message??"the result did not satisfy the result contract"):writingSpec?specContinuation(specVersion):route?consultationContinuation(route):answerContinuation();
