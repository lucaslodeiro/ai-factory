...
export function workflowNextStep(store:Store,id:string,status:string,pr?:string){
 ...
 if(type==='brief-approval')return {title:'Brief waiting for your approval',url:null,respondLabel:'Approve or request changes',command:`/factory approve v${request.specVersion}`};
 if(type==='spec-approval')return {title:'Prototype waiting for your approval',url:null,respondLabel:'Approve or request changes',command:`/factory approve v${request.specVersion}`};
