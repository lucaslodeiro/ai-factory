...
import {reportedTokens,sinceSessionTotal,tokenUsageReducer,type TokenUsage} from "./token-usage.js";
...
export function progressMonitor(logDir:string,provider:AgentProvider|null,sessionBaseline?:Partial<TokenUsage>|null){
 ...
 const event=(value:Record<string,unknown>,at:string)=>{
  ...
  usage.add(value);progress.usageTokens=reportedTokens(sinceSessionTotal(usage.result(),sessionBaseline),provider);
