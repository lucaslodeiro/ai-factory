...
export function progression(rows:ActivityRow[]):RunProgression[] {
  ...
  for (const [role,unsorted] of byRole) {
    ...
    // Tokens are the comparison: every provider reports them, and they mean the same thing for each.
    // A run that reported none compares as unknown rather than as an improvement.
    const basis=(row:ActivityRow)=>row.totalTokens;
    ...
    runs.forEach((row,index)=>{
      ...
      result.push({role,run:index+1,stage:row.stage,turns:row.turns,events:row.events,
        totalTokens:row.totalTokens,durationMs:row.durationMs,
...
export interface RoleActivity {
  ...
  cacheReadTokens:number|null; cacheWriteTokens:number|null; outputTokens:number|null;
  totalTokens:number|null; topTypes:string;
}
...
export function summarizeActivity(rows:ActivityRow[]):RoleActivity[] {
  ...
  return [...byRole.entries()].map(([role,runs])=>{
    const sum=(field:"events"|"turns"|"cacheReadTokens"|"cacheWriteTokens"|"outputTokens"|"totalTokens")=>runs.reduce<number|null>((total,row)=>add(total,row[field]),null);
  ...
  // The objective is stated in tokens, and event counts are shaped by each provider's stream, so an
  // events-first ordering compares formats rather than work.
...
export function activityRow(payload:unknown, role:string, stage:string|null, startedAt:string|null):ActivityRow {
  ...
  return {role,stage,startedAt,events:number(activity.events),eventTypes:types,turns:number(activity.turns),
    ...
    totalTokens:number(usage.totalTokens),durationMs:number(activity.durationMs)};
}
