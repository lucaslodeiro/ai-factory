...
import {budgetState,liveBudget,formatTokens,budgetGracePercent} from './budget.js';
export function workflowActivity(store:Store,id:string,status:string,now=Date.now()){
 const run=store.db.prepare('SELECT e.id,e.role,e.status,e.started_at,e.finished_at FROM executions e JOIN work_items w ON w.active_run_id=e.id WHERE w.id=?').get(id) as {id:string;role:string;status:string;started_at:string;finished_at:string|null}|undefined;
 ...
 if(status==='RUNNING'){
  ...
  if(run.status!=='running'){
   ...
   return {label:stale?'Needs recovery':'Processing result',role:run.role,detail:stale?'The agent has finished, but the workflow has not advanced.':`Agent finished (${run.status}); applying its result.`,since:run.finished_at,stalled:stale};
  }
  ...
  if(live.alert)return{label:'Token budget reached',role:run.role,detail:`${formatTokens(live.consumed!)} of ${formatTokens(budget.granted)} tokens used. You can pause this execution now; otherwise it may finish within the ${budgetGracePercent}% grace and Factory stops it at ${100+budgetGracePercent}%.`,since:run.started_at,lastProgressAt:progress?.lastProgressAt??null,progressEvents:progress?.events??0,warning:true,stalled:false};
  ...
  return {label:warning?'Check agent progress':'Agent running',role:run.role,detail:warningText,since:run.started_at,lastProgressAt:progress?.lastProgressAt??null,progressEvents:progress?.events??0,warning:Boolean(warning),stalled:false};
 }
