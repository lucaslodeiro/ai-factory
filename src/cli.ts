...
for (const name of ["cancel", "retry", "refresh"] as const) p.command(name).argument("<id>").description(name === "refresh" ? "Reload the issue and evaluate only its latest comment" : `${name} a work item`).action(id => {
