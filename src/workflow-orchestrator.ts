...
export class WorkflowOrchestrator {
 ...
 async continueIssue(reference:string){const issue=await this.github.issue(this.issueNumber(reference)),state=await readIssueState(this.github,issue.number);if(!state)throw new Error("No readable Factory state is published for this issue");const projection=adoptIssueState(this.store,state,{issue});this.store.event("issue.continued",{issue:issue.number,instance:state.index.instance,revision:state.index.projection.revision,forced:true},state.index.workflowId);return{issue:issue.number,workItemId:state.index.workflowId,status:projection.status,sourceInstance:state.index.instance};}
 private async reconcileAssignments(login:string){
  ...
  for(const issue of this.assigned){
   ...
   if(local){
    ...
    if(["COMPLETED","CANCELLED"].includes(local.status)){if(!this.released(local))try{await this.github.unassign(local.issue_number,[login]);await this.github.removeLabel(local.issue_number,own);this.updateContext(local.id,{releasedAt:new Date().toISOString()});}catch(error){if(this.issueNotFound(error)){await this.archiveDeleted(local);continue;}throw error;}view.push({issue:issue.number,state:"released",instances:[]});continue;}
  ...
  for(const local of this.rows().filter(item=>!item.archived_at&&!assignedIds.has(item.issue_id??-1))){
   if(["COMPLETED","CANCELLED"].includes(local.status)){if(!this.released(local))try{await this.github.unassign(local.issue_number,[login]);await this.github.removeLabel(local.issue_number,own);this.updateContext(local.id,{releasedAt:new Date().toISOString()});}catch(error){if(this.issueNotFound(error)){await this.archiveDeleted(local);continue;}throw error;}continue;}
 ...
 private issueNotFound(error:unknown){return /\bHTTP\s*(?:404|410)\b|Could not resolve to an issue or pull request with the number/i.test(error instanceof Error?error.message:String(error));}
