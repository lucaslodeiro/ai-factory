...
export class WorkflowOrchestrator {
 ...
 startIssue(reference:string,requestedBy="Dashboard or CLI",origin?:{source:"comment"|"description";commentId?:number;login:string;guidance?:string},knownIssue?:Issue){
  const number=this.issueNumber(reference),issue=knownIssue??this.github.issue(number),candidate=this.rows().find(item=>item.repo===config.repo&&item.issue_number===number&&!item.archived_at);
 ...
 private discoverStartCommands(){
  if(!this.github.repositoryComments)return;const key=`github.start-comments:v2:${config.repo}`,now=Date.now(),checkpoint=this.store.metadata<{since:string;evaluated:Record<string,string>}>(key)??{since:new Date(0).toISOString(),evaluated:{}};const evaluated={...checkpoint.evaluated};
  const issues=new Map<number,Issue>();for(const comment of this.github.repositoryComments(checkpoint.since).slice().sort((a,b)=>a.id-b.id)){const issueNumber=Number(comment.issueUrl.split("/").at(-1));if(!Number.isSafeInteger(issueNumber))continue;let issue=issues.get(issueNumber);if(!issue){issue=this.github.issue(issueNumber);issues.set(issueNumber,issue);}if(this.tracked(issue))continue;if(evaluated[String(comment.id)]===comment.updatedAt)continue;evaluated[String(comment.id)]=comment.updatedAt;this.startComment(comment,issue);}
