...
export class WorkflowOrchestrator {
 ...
 private startComment(comment:RepositoryComment){let command;try{command=parseFactoryCommand(comment.body);}catch(error){this.store.event("command.rejected",{commentId:comment.id,login:comment.user.login,error:String(error)});return;}const issueNumber=Number(comment.issueUrl.split("/").at(-1));if(command?.kind!=="start"){if(!command&&this.misplacedStart(comment.body)&&comment.user.type==="User"&&config.approvers.includes(comment.user.login)&&Number.isSafeInteger(issueNumber)){const issue=this.github.issue(issueNumber);if(!this.tracked(issue)){const reason=this.startPlacementHint();this.store.event("command.rejected",{commentId:comment.id,issueNumber,login:comment.user.login,command:"start",error:reason});this.publishStartHint(issueNumber,`comment-${comment.id}-${comment.updatedAt}`,reason);}}return;}if(comment.user.type!=="User"||!config.approvers.includes(comment.user.login)||!Number.isSafeInteger(issueNumber)){this.store.event("start.command_rejected",{commentId:comment.id,issueNumber,login:comment.user.login,reason:"Only an authorized human can start work"});return;}try{this.startIssue(String(issueNumber),comment.user.login,{source:"comment",commentId:comment.id,login:comment.user.login,guidance:command.guidance});}catch(error){this.store.event("start.command_rejected",{commentId:comment.id,issueNumber,login:comment.user.login,reason:String(error)});}}
