...
// The split was approved as a decision in the brief; the spec shows every story, the criteria it
// owns and what it waits for.
function storiesMarkdown(result:Pick<AgentResult,"stories"|"acceptanceCriteria">){const cell=(value:string)=>value.replaceAll("|","\\|"),owned=new Set(result.stories.flatMap(story=>story.criteria)),epic=result.acceptanceCriteria.filter(criterion=>!owned.has(criterion.id)).map(criterion=>criterion.id);
 return `## Stories\n\nEach story is delivered by the Builder and the Tester on its own branch from this issue's branch; the Reviewer reviews the whole once every story is integrated.\n\n| Story | Scope | Criteria | Waits for | Risk · depth |\n| --- | --- | --- | --- | --- |\n${result.stories.map(story=>`| **${cell(story.key)}** ${cell(story.title)} | ${cell(story.scope)} | ${story.criteria.join(", ")} | ${story.dependsOn.join(", ")||"—"} | ${story.assessment.risk} · ${story.assessment.verificationDepth} |`).join("\n")}${epic.length?`\n\nVerified on the whole, after every story: ${epic.join(", ")}.`:""}`;}
...
const briefApprovalAction=(specVersion:number)=>`## Next action\n\n**Approve brief v${specVersion}**\n\n\`/factory approve v${specVersion} [guidance]\`\n\nApproving accepts every recommendation above. Architect then writes the detailed specification and its acceptance criteria; you do not need to read it. If the change has significant UX impact, a Designer prototypes it and you approve the prototype before the Builder starts. Optional guidance becomes a SPEC-scoped instruction.\n\n**Change a decision or request changes**\n\n\`/factory answer <feedback>\`\n\nFeedback becomes a human decision for Architect, who proposes a new brief.`;
const prototypeApprovalAction=(specVersion:number)=>`## Next action\n\n**Approve the prototype and SPEC v${specVersion}**\n\n\`/factory approve v${specVersion} [guidance]\`\n\nThe Builder starts from the approved specification and these screenshots. Optional guidance becomes a SPEC-scoped instruction.\n\n**Request changes**\n\n\`/factory answer <feedback>\`\n\nFeedback becomes a human decision for Architect, who proposes a new brief.`;
...
function prototypeMarkdown(result:AgentResult,prototype?:{repo:string;head:string},imageUrl?:(file:string)=>string) {
 ...
 const shots=images.map(file=>{const url=imageUrl?imageUrl(file):link(file)?`${link(file)}?raw=true`:undefined,name=file.split("/").at(-1)!;return url?`**${name}**\n\n![${name}](${url})`:`- \`${file}\``;}).join("\n\n");
 return `## Screenshots\n\n${shots}${notes?`\n\nStates, flow and copy: ${link(notes)?`[${notes}](${link(notes)})`:`\`${notes}\``}`:""}`;
}
export function resultMarkdown(role:AgentRole,result:AgentResult,specVersion:number,pullRequestUrl?:string,options:{reportOnly?:boolean;prototype?:{repo:string;head:string};prototypeFollows?:boolean;imageUrl?:(file:string)=>string}={}) {
 result=publishedAgentData(result);
 const designer=role==="designer"&&result.outcome==="pass",architect=role==="product-architect",prototypeFollows=architect&&result.outcome==="spec"&&Boolean(options.prototypeFollows);
 const environmentBlocked=result.findings.some(finding=>finding.classification==="environment-blocked");
 const heading=architect&&result.outcome==="questions"?(environmentBlocked?"Architect — blocked":"Architect — questions"):architect&&result.outcome==="resolved"?"Architect — tactical decision":architect&&result.outcome==="brief"?`Brief v${specVersion} — awaiting approval`:architect?(prototypeFollows?`Specification v${specVersion} — prototype in progress`:`Specification v${specVersion} — delivery started`):designer?`Prototype for SPEC v${specVersion} — awaiting approval`:`${roleFullName(role)} report`;
 const sections=[`# ${heading}`,`${options.reportOnly&&role==="qa"?"## Tester summary":"## Summary"}\n\n${result.summary}`];
 if(architect&&result.outcome==="brief")sections.push(result.brief.replace(/^(#{1,5})(?=\s)/gm,"#$1"),briefApprovalAction(specVersion));
 if(architect&&result.outcome==="spec")sections.push(`## Acceptance criteria\n\n| ID | Criterion |\n| --- | --- |\n${result.acceptanceCriteria.map(criterion=>`| ${criterion.id} | ${criterion.description.replaceAll("|","\\|")} |`).join("\n")}`,...(result.stories.length?[storiesMarkdown(result)]:[]),`## Next action\n\n> ${prototypeFollows?"The change has significant UX impact, so the Designer is preparing a prototype of this specification. Its approval command arrives with the screenshots; there is nothing to do yet.":result.stories.length?"The stories are being created as sub-issues and start as their dependencies close. No human action is required.":"The Builder starts from this specification. No human action is required."}`,`<details>\n<summary>Full technical specification v${specVersion} (for Builder, Tester and Reviewer)</summary>\n\n${result.spec.replace(/^(#{1,5})(?=\s)/gm,"#$1")}\n\n</details>`);
 if(designer)sections.push(prototypeMarkdown(result,options.prototype,options.imageUrl),prototypeApprovalAction(specVersion));
 ...
 if(result.testCandidates.length){const kept=result.testCandidates.filter(candidate=>candidate.kept),discarded=result.testCandidates.filter(candidate=>!candidate.kept);sections.push(`## Test selection\n\n${kept.length} of ${result.testCandidates.length} candidates kept.\n\n| Test | Covers | Value | Why |\n| --- | --- | --- | --- |\n${kept.map(candidate=>`| ${candidate.name.replaceAll("|","\\|")} | ${candidate.covers.join(", ")} | ${candidate.value} | ${candidate.reason.replaceAll("|","\\|")} |`).join("\n")}${discarded.length?`\n\n<details>\n<summary>Discarded (${discarded.length})</summary>\n\n${discarded.map(candidate=>`- **${candidate.name.replaceAll("|","\\|")}** (${candidate.value}, covers ${candidate.covers.join(", ")}) — ${candidate.reason}`).join("\n")}\n\n</details>`:""}`);}
...
export function isMilestoneResult(store:Store,row:{work_item_id:string;run_id:string},payload:{role:AgentRole;result:AgentResult}) {
 ...
 if(payload.role==="product-architect"&&["brief","spec","questions","resolved"].includes(payload.result.outcome))return true;
...
export class WorkflowGitHubPublisher {
 ...
 async publishResults() {
  ...
  for(const row of rows) {
   ...
   const specification=payload.role==="product-architect"&&["brief","spec"].includes(payload.result.outcome);let prototypeFollows=false;
   if(specification){
    const spec=this.store.db.prepare("SELECT version,body,criteria,stories,assessment FROM specs WHERE work_item_id=? AND version=?").get(row.work_item_id,version) as {version:number;body:string;criteria:string;stories:string;assessment:string|null};const assessment=spec.assessment?JSON.parse(spec.assessment) as {uxImpact?:string}:null;prototypeFollows=assessment?.uxImpact==="significant";machine={kind:"spec",version:spec.version,body:spec.body,criteria:JSON.parse(spec.criteria),stories:JSON.parse(spec.stories),assessment};
