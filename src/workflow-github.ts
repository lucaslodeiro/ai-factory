...
// The split is approved with the brief, so the human sees every story, what it owns and what it
// waits for, in the same comment as the approval command.
 ...
 async publishResults() {
  ...
  for(const row of rows) {
   ...
   if(payload.role==="product-architect"&&payload.result.outcome==="spec"){
    const spec=this.store.db.prepare("SELECT version,body,criteria,stories,assessment FROM specs WHERE work_item_id=? AND version=?").get(row.work_item_id,version) as {version:number;body:string;criteria:string;stories:string;assessment:string|null};machine={kind:"spec",version:spec.version,body:spec.body,criteria:JSON.parse(spec.criteria),stories:JSON.parse(spec.stories),assessment:spec.assessment?JSON.parse(spec.assessment):null};
