...
function resultMarkdown(role:AgentRole,result:AgentResult,specVersion:number) {
 const heading=role==="product-architect"&&result.outcome==="questions"?"Architect — questions":role==="product-architect"?`Specification v${specVersion}`:`${roleFullName(role)} report`;
 ...
 if(!result.questions.length){const action=role==="product-architect"?"Review the specification and use the command shown in the AI Factory status comment.":role==="reviewer"?"Review the pull request and merge it when ready.":`${roleShortName(role)} finished. The next workflow stage is queued automatically.`;sections.push(`## Next action\n\n> ${action}`);}
