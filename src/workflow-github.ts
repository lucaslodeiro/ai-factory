...
export function resultMarkdown(role:AgentRole,result:AgentResult,specVersion:number,pullRequestUrl?:string,options:{reportOnly?:boolean;prototype?:{repo:string;head:string};prototypeFollows?:boolean}={}) {
 ...
 if(designer)sections.push(prototypeMarkdown(result,options.prototype),prototypeApprovalAction(specVersion));
