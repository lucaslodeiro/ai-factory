...
export function browserInstructions(cwd,runtimeUrl){
 ...
 const runtime=runtimeUrl?`Factory has already started and is supervising the local runtime at ${runtimeUrl}. Use that URL; do not start another preview server.`:`${preview} Start it in the background with its input and output redirected (for example \`npm run ${script??'dev'} </dev/null >/tmp/preview.log 2>&1 &\`), wait for its loopback port, connect to it over 127.0.0.1, and stop it before you return your result. A server still attached to your tool's output keeps the provider from finishing your turn.`;
 return `\n\nFactory browser runner:\nRead process.env.FACTORY_BROWSER_REPORT for the host supervisor readiness evidence. If FACTORY_BROWSER_STATUS is unavailable, the report explains why; do not assume the browser is ready.\nUse process.env.FACTORY_BROWSER_CDP_URL with playwright.chromium.connectOverCDP() instead of chromium.launch(). For Lighthouse use Number(process.env.FACTORY_BROWSER_DEBUG_PORT) as its port; do not call chrome-launcher. Adapt browser test scripts to support these environment variables while retaining standalone local launch as fallback. ${runtime} Never register a system service or use launchctl, systemctl, service, pm2, or a persistent local-start script to run a preview. This browser uses a fresh temporary profile, with no user cookies or credentials. Do not navigate to file:// URLs or read host files through the browser. The supervisor owns browser lifecycle: never kill the browser process or send the raw CDP Browser.close command. Close your own contexts, and end every script that connected with \`await browser.close()\`: on a connectOverCDP connection it only disconnects and leaves the browser running, and without it the open connection keeps your script, and the command that ran it, from ever exiting. These probes establish infrastructure readiness only; run all required project checks and record their evidence. If the connection fails, report environment-blocked with the actual error.\n`;}
