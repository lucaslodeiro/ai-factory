...
export async function startDaemon(store = new Store(),github=new GitHubAdapter(),options:{controller?:ControllerLease}={}) {
 ...
 let controllerState:LeaseObservation=initialController,controllerMode:"active"|"standby"|"uncertain"|"fenced"=initialController.state==="active"?"active":"standby",lastControllerSuccess=performance.now(),generation=initialController.state==="absent"?0:initialController.record.generation;
 ...
 try {
  ...
  let renewalBusy=false;const renewController=()=>{if(renewalBusy)return;renewalBusy=true;try{if(controllerMode==="active"||controllerMode==="uncertain"){const verified=controller.assertController(generation);controllerState=controller.renew(verified);lastControllerSuccess=performance.now();if(controllerMode==="uncertain"){controllerMode="active";runner.applyHeld();store.event("controller.ownership_restored",{generation});}}else{controllerState=controller.readLease();if(controllerState.state==="active"){generation=controllerState.record.generation;controllerMode="active";lastControllerSuccess=performance.now();store.event("controller.acquired",{repository:repository.fullName,displayName:controller.instance.displayName,generation});}}}catch{try{const observed=controller.readLease();if(observed.state==="absent"||observed.record.instanceId!==controller.instance.instanceId||observed.record.generation!==generation){loseController();return;}}catch{}if(performance.now()-lastControllerSuccess>=600_000&&controllerMode==="active"){controllerMode="uncertain";controller.markLocalState("uncertain","Renewal could not be verified");store.event("controller.ownership_uncertain",{generation});}}finally{renewalBusy=false;}};
