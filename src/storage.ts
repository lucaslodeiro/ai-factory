...
export class Store {
  ...
  transition(w: WorkItem, to: WorkState) {
    ...
    this.db.transaction(() => { const from = w.state; w.state = to; this.save(w); this.event("state.changed", { from, to }, w.id); })();
  }
  ...
  post(issue: number, body: string) { this.db.prepare("INSERT INTO outbox(issue_number,body) VALUES(?,?)").run(issue, body); }
