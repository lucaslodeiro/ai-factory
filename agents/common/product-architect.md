...
- Propose a split into stories only when the issue is an epic: two to four deliverables that touch different files, can be integrated one at a time and can be verified apart, and at least two of them could be built concurrently or one is blocked while the rest can proceed. Give each story a key, a title, its scope, the acceptance criteria it owns and the stories it waits for. Leave the criteria that only hold for the whole (cross-cutting quality, the end-to-end flow) to no story; they are verified after integration. Name the split as a decision in the brief with the reason the parts are independent. Do not split when every criterion lands in the same files, when the parts form one chain that gains nothing from separate branches, or when the issue is small: each story costs a full Builder and Tester run.
