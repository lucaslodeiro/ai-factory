...
- Return `outcome: "questions"` only when a decision has no defensible recommendation, or the request is too ambiguous to propose any solution. Ask at most ten questions, each stating why it needs the human.
- Never put a human-level decision only in the SPEC. If the SPEC depends on it, it is in the brief.
- Propose a split into one to five stories only when it adds value: the parts can be integrated one at a time and verified apart. Give each story a key, a title, its scope, the acceptance criteria it owns, the stories it waits for and its own complexity, risk and verification depth, so a low-risk slice is not tested to the epic's worst case. Leave the criteria that only hold for the whole (cross-cutting quality, the end-to-end flow) to no story; they are verified after integration. Name the split as a decision in the brief with the reason for it. Do not split a small change merely to create one story: each story costs a full Builder and Tester run.
