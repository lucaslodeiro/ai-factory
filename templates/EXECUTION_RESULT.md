...
| `brief` | Only for a new specification: the document the human approves instead of reading the SPEC (decisions with recommendations, solution, criteria, assumptions, assessment). Aim for 5000 characters; length alone never invalidates a completed proposal. |
| `spec`, `acceptanceCriteria` | Only for a new specification: markdown plus unique IDs/descriptions. |
| `stories` | Only for a new specification, and empty unless splitting adds value: 1 to 5 stories with a unique `key` and `title`, a `scope`, the `criteria` ids the story owns (each id in at most one story), `dependsOn` keys forming no cycle and an `assessment` (complexity, risk, verificationDepth floored by the worse of the two) that governs that story's Tester. Builder and Tester deliver each story on its own branch from the epic branch; criteria no story owns are verified on the whole once every story is integrated. |
