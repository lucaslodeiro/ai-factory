...
try{
  ...
  fs.writeFileSync(path.join(engine,"package.json"),JSON.stringify({name:"ai-factory"}));fs.copyFileSync(path.join(source,"scripts/uninstall.mjs"),path.join(engine,"scripts/uninstall.mjs"));fs.copyFileSync(path.join(source,"scripts/ai-factory"),path.join(engine,"scripts/ai-factory"));fs.symlinkSync(path.join(source,"node_modules"),path.join(engine,"node_modules"),"dir");
  ...
  const bin=path.join(sandbox,"bin"),gitLog=path.join(sandbox,"git.log"),launchctlLog=path.join(sandbox,"launchctl.log"),launchctlState=path.join(sandbox,"launchctl-state");fs.mkdirSync(bin);fs.mkdirSync(launchctlState);fs.writeFileSync(path.join(bin,"git"),`#!/bin/sh\nprintf '%s\\n' "$*" >> "$GIT_LOG"\nif [ "$1" = ls-remote ]; then exit 0; fi\nexec /usr/bin/git "$@"\n`,{mode:0o755});
  fs.writeFileSync(path.join(bin,"launchctl"),`#!/bin/sh\nprintf '%s\\n' "$*" >> "$LAUNCHCTL_LOG"\ncase "$1" in\nlist) for file in "$LAUNCHCTL_STATE"/*; do [ -e "$file" ] && printf '1\\t0\\t%s\\n' "\${file##*/}"; done;;\nprint) [ -f "$LAUNCHCTL_STATE/\${2##*/}" ];;\nbootout|remove) rm -f "$LAUNCHCTL_STATE/\${2##*/}";;\nesac\n`,{mode:0o755});
  ...
  for(const label of["com.ai-factory.daemon","com.ai-factory.dashboard","com.ai-factory.update.123"])fs.writeFileSync(path.join(launchctlState,label),"");
  const env={...process.env,PATH:`${bin}:${process.env.PATH}`,HOME:userHome,GIT_LOG:gitLog,LAUNCHCTL_LOG:launchctlLog,LAUNCHCTL_STATE:launchctlState,AI_FACTORY_HOME:home};
  ...
  assert.match(fs.readFileSync(launchctlLog,"utf8"),/bootout .*com\.ai-factory\.daemon/);assert.match(fs.readFileSync(launchctlLog,"utf8"),/bootout .*com\.ai-factory\.dashboard/);assert.match(fs.readFileSync(launchctlLog,"utf8"),/remove com\.ai-factory\.update\.123/);assert.deepEqual(fs.readdirSync(launchctlState),[]);
  assert.equal(fs.existsSync(engine),false);assert.equal(fs.existsSync(data),false);assert.equal(fs.readFileSync(path.join(home,".env"),"utf8").includes("FACTORY_DATA_DIR"),true);assert.equal(fs.readFileSync(path.join(target,"keep"),"utf8"),"target");assert.match(basic.stdout,new RegExp(path.join(home,".env").replace(/[.*+?^${}()|[\]\\]/g,"\\$&")));assert.match(basic.stdout,/uninstall helper remains available/);assert.equal(fs.realpathSync(path.join(userHome,".local","bin","ai-factory")),fs.realpathSync(path.join(home,".uninstall","scripts","ai-factory")));
  ...
  const sequentialHome=path.join(userHome,"sequential"),sequentialEngine=path.join(sequentialHome,"engine"),sequentialLauncher=path.join(userHome,".local","bin","ai-factory");fs.mkdirSync(path.join(sequentialEngine,"scripts"),{recursive:true});fs.writeFileSync(path.join(sequentialEngine,"package.json"),JSON.stringify({name:"ai-factory"}));fs.copyFileSync(path.join(source,"scripts/uninstall.mjs"),path.join(sequentialEngine,"scripts","uninstall.mjs"));fs.copyFileSync(path.join(source,"scripts/ai-factory"),path.join(sequentialEngine,"scripts","ai-factory"));fs.symlinkSync(path.join(source,"node_modules"),path.join(sequentialEngine,"node_modules"),"dir");fs.writeFileSync(path.join(sequentialHome,".env"),"FACTORY_DATA_DIR=data\n");fs.symlinkSync(path.join(sequentialEngine,"scripts","ai-factory"),sequentialLauncher);
  const sequentialBasic=run(sequentialLauncher,["uninstall","--yes"],{cwd:userHome,env:{...env,AI_FACTORY_HOME:sequentialHome}});assert.equal(sequentialBasic.status,0,sequentialBasic.stderr+sequentialBasic.stdout);assert.equal(fs.existsSync(path.join(sequentialHome,".env")),true);assert.equal(fs.existsSync(sequentialEngine),false);
  const sequentialPurge=run(sequentialLauncher,["uninstall","--purge","--yes"],{cwd:userHome,env:{...env,AI_FACTORY_HOME:sequentialHome}});assert.equal(sequentialPurge.status,0,sequentialPurge.stderr+sequentialPurge.stdout);assert.equal(fs.existsSync(sequentialHome),false);assert.equal(fs.existsSync(sequentialLauncher),false);
