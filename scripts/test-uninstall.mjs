...
try{
  ...
  const purgeBlocked=run(process.execPath,[path.join(engine,"scripts/uninstall.mjs"),"--purge","--yes"],{cwd:engine,env});assert.notEqual(purgeBlocked.status,0);assert.match(purgeBlocked.stdout,/dirty working tree/);assert.equal(fs.existsSync(home),true);
