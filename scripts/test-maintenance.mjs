...
for(const f of ['install-core.sh','update.sh','update.mjs','configure.sh','configure.mjs','dashboard-url.mjs','prepare-dashboard-config.mjs','initialize-environment.mjs','paths.mjs','validate-installation.mjs']) fs.copyFileSync(path.join(source,'scripts',f),path.join(seed,'scripts',f));
...
const installedIdentity=JSON.parse(fs.readFileSync(path.join(dest,'instance.json'),'utf8'));assert.equal(installedIdentity.schemaVersion,1);assert.match(installedIdentity.displayName,/^Factory [0-9a-f]{6}$/);
...
const priorRevision=run('git',['rev-parse','HEAD'],engine).stdout.trim();
env.FAIL_NPM_BUILD='1';
const failedCandidate=run('bash',['scripts/update.sh'],engine,false);
assert.match(failedCandidate.stderr,/building the candidate/);
assert.equal(run('git',['rev-parse','HEAD'],engine).stdout.trim(),priorRevision);
assert.equal(fs.existsSync(path.join(engine,'change.txt')),false);
assert.equal(fs.readFileSync(path.join(dest,'.env'),'utf8'),originalEnv);
assert.equal(fs.existsSync(path.join(engine,'dist','src','cli.js')),true);
assert.equal(JSON.parse(fs.readFileSync(env.AI_FACTORY_UPDATE_STATE_FILE,'utf8')).versionActivated,false);
delete env.FAIL_NPM_BUILD;
env.FAIL_INSTALL_CHECK='1';
const failedValidation=run('bash',['scripts/update.sh'],engine,false);
assert.match(failedValidation.stderr,/validating the candidate installation/);
assert.equal(run('git',['rev-parse','HEAD'],engine).stdout.trim(),priorRevision);
assert.equal(fs.existsSync(path.join(engine,'change.txt')),false);
...
assert.deepEqual(JSON.parse(fs.readFileSync(path.join(dest,'instance.json'),'utf8')),installedIdentity);
