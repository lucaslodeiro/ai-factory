...
assert.deepEqual(Object.keys(JSON.parse(fs.readFileSync(path.join(dest,'.factory','install.json'),'utf8'))).sort(),['branch','installedAt','revision','version']);
...
assert.equal(fs.existsSync(path.join(failedDest,'.factory','install.json')),false);
...
assert.equal(JSON.parse(fs.readFileSync(path.join(dest,'.factory','install.json'),'utf8')).revision,run('git',['rev-parse','HEAD'],dest).stdout.trim());
