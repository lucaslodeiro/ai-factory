...
The dashboard maintains a small, explicit list of model IDs for each provider. It always offers `auto` once and accepts a custom ID when a model is missing from the list. These are suggestions, not a claim that every connected account can use every model: check [OpenAI models](https://developers.openai.com/api/docs/models), [Claude CLI model selection](https://code.claude.com/docs/en/cli-usage), or [Cursor CLI model selection](https://cursor.com/docs/cli/overview) for current availability. With `auto`, the factory omits the model override. A rejected model fails the run; the factory never silently changes provider or model. Changing role settings requires restarting the daemon and affects future attempts.
