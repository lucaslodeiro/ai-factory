...
The status table identifies the active repository controller by its neutral
factory display name. Its footer includes the controller generation beside the
workflow and presentation revisions.
...
Because editing the status comment does not create a GitHub notification, the publisher assigns the issue to the configured approvers while human action is required: a human-owned `WAITING` request, any `FAILED` state, or `PAUSED`/`CANCELLED` entered by a non-human actor. It removes only those configured approver assignments when the item leaves that condition and preserves unrelated assignees. The publisher reads the current assignee set first and performs no add/remove call when it already matches.

### 8.3 Milestone comments (immutable)

Each milestone comment ends with the repository controller display name so its
origin remains visible after a takeover.

Only when a human acted or must act, or the contract changed: started, questions, SPEC proposed, SPEC approved, decision request opened, decision resolved, correction limit, failure needing retry, paused, cancelled, ready to merge, PR closed, recovered, merged. Architect question comments are titled as questions, number each question and include a ready-to-copy `/factory answer` template; they never describe nonexistent content as a specification. A proposed SPEC comment identifies that approval is pending, demotes its internal headings below the comment title, renders the structured acceptance criteria, and includes concrete approve and feedback commands with their semantics. Tactical-resolution comments are titled as Architect decisions, name the role that continues and require no human action; while that result is current, its summary replaces the earlier delivery-role question in the mutable status. A delivery-role decision says Architect continues without human action, and the ready-to-merge Reviewer milestone includes the pull-request URL. Role `pass` reports and intermediate `changes` reports go to the status comment (and the PR body for the final ones), not to immutable comments.
...
16. **Cutover guard.** A V3 daemon refuses a database without schema version 4 and tells the operator to uninstall or select an empty data directory. A fresh database receives the version only after schema creation succeeds. No legacy rows are read or changed.
