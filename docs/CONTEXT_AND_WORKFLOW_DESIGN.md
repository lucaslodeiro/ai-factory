...
16. **Cutover guard.** A V3 daemon refuses a database without schema version 9 and tells the operator to uninstall or select an empty data directory. A fresh database receives the version only after schema creation succeeds. No legacy rows are read or changed.
