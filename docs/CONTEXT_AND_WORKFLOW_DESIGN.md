...
It is given to the Builder alone on purpose. The Tester and the Reviewer run the same issue without it, so `execution.finished` compares a role that has the map against roles that do not, on the same work, without a feature flag or a code revert. **Compare the right field for the provider**: on Codex the event histogram is real, because it streams one JSON object per line; on Claude it is always 1, because it returns a single result envelope, and the comparable number there is the turn count it reports. An installation whose roles all run on Claude must read `turns`, never `events`.
...
Every GitHub write is recorded under a publication key in `metadata`: `github:status:<workItemId>` for the status comment, `github:result:<eventId>` for a milestone comment, `github:failure:<failureId>` for a failure comment and `github:help:<workItemId>` for the command reference. A key holds `published` with the confirmed comment id, its URL and the confirmation time, `failed` with the attempt count and the last error, or `skipped` for a result that is not a milestone. A write is `published` only after GitHub returned the comment; a failure keeps the local event untouched and is retried with the same marker key, so recovery never duplicates a comment. `flush()` runs the help, milestone and status passes independently and each pass visits every work item before rethrowing the first error, so one unreachable comment never blocks the other issues' status. The first failure of a key records `github.publish_failed` with the work item; the third consecutive failure records `github.publish_stalled` once, which the dashboard shows as needing attention. Success stores the comment id returned by the adapter and records `github.published` for the work item, which is what lets an open dashboard conversation flip the milestone from pending to published; the events feed does not list it. `syncWorkflow` returns the status comment id for that purpose.
...
The dashboard's System activity panel lists only events no issue conversation shows: GitHub synchronization and publication problems, issue lifecycle changes seen on GitHub, claims and continuations, maintenance, notifications, controls and runtime problems. Workflow transitions, agent results, executions, commands and model selection are excluded because they belong to an issue's conversation; any other event type appears by default. The dashboard issue detail presents one ordered conversation derived from existing execution and workflow events. Each agent execution is one turn: its header shows role, outcome or process state, provider, model, start time and duration (from the `executions` row, or from the event timestamps when a continued item has none); its body shows the result through the GitHub result renderer, or the running or failure state, and a collapsed Agent input block with the bounded manifest. Exact prompt text remains local and requires explicit acknowledgement. Event turns show transitions and failure evidence, and human turns identify whether they came from an issue comment or the dashboard. Turns carry explicit Agent execution, Workflow and Human guidance labels. Every turn also carries a `milestone` flag: an execution whose result GitHub gets as a permanent comment (the same rule the publisher uses) or whose process did not finish cleanly, any human guidance, and a transition into a state that waits on a person or ends the work (waiting, failed, paused, cancelled, completed), excluding the continuation transition. The continuation transition is not a turn: the conversation header states the source installation, revision and time of the latest continuation, says when the earlier turns live on that installation because none were recorded locally, and counts repeated moves. Continuation events carry the source `instance`, `revision` and `publishedAt` in their reason. The conversation offers a "Milestones and human guidance only" filter, off by default and remembered per browser, that hides everything else and shows how many turns it hid. Live refresh does not replace an unchanged issue list or unchanged conversation; when data changes it preserves open issue panels, open prompt summaries and unsent guidance. The conversation grows with the page instead of scrolling inside it. By default only the newest turn with a body opens; a transition without details never does. Turn times are relative within the last hour and the clock time after that, with the date when it is not today. An open conversation reloads when the work item's last event id changes, not only when a transition bumps its revision, so prompts, inbox comments and notes appear without a page reload. Each publishable milestone turn (Architect spec, questions and decisions, delivery-role decisions, Reviewer approval, correction limits and failures) carries its publication state: pending until GitHub confirmed the comment, published with a link to that comment, or not published with the stored error and attempt count. The conversation header notes, as one row with a link to the comment, when the issue status comment is behind the local presentation revision or its last publication failed, and says what happens next: a live countdown to the next GitHub cycle (the daemon records `nextAt` in `runtime:github-sync` when each remote cycle ends), "Retrying now" while one runs, or that the daemon is stopped; after three failed attempts it also says to check GitHub access. While the conversation shows a failed or lagging status comment it reloads after every GitHub cycle, because only the first and third failures record an event. Error URLs stay in the tooltip. The happy path shows nothing. Every command successfully applied by the GitHub inbox emits `command.applied` with its comment id, login, command label, carried text and `source: "comment"`, so approvals, answers and notes appear in that conversation just like dashboard messages. The issue card's next step pairs its GitHub link with a dashboard action (Request changes, Approve or request changes, Answer or Give guidance) that opens the conversation and focuses the composer, and shows the equivalent `/factory` command as one line for people who prefer the issue. Below it, the card adds one activity line only when it says something the state chip does not: why queued work is not starting, how long an agent has been running, a recovery need or a pause reason. A failure keeps its expanded diagnosis and a pause lists its open findings. The composer derives its buttons only from the current projection and active request. The authenticated `gh` login must be an approver. For every dashboard message the daemon publishes an idempotent command-form comment first, including the login and instance, then applies the same `WorkflowCommands` path with the real GitHub comment id. Factory markers prevent the inbox from applying that comment twice. A rejected command is appended to that published comment and recorded as a failed control.
...
16. **Cutover guard.** A V3 daemon refuses a database without schema version 10 and tells the operator to uninstall or select an empty data directory. A fresh database receives the version only after schema creation succeeds. No legacy rows are read or changed.
