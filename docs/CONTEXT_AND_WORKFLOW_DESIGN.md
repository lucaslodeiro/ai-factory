...
The status table includes the active Factory instance name. Its footer includes `<sub>instance:<name></sub>` beside the workflow attribution, so ownership remains visible when several installations share the repository.
...
Each milestone comment ends with `<sub>instance:<name></sub>` so its origin remains visible after ownership moves.
