...
Claude 2.1.281: **PASS**. The killed run's stream ended after the first tool result with no `result` event; the resumed run answered with that file's word without reading any file, for USD 0.011. Codex and Cursor: not yet run, since neither is authenticated in this environment. Run on the installation with `node ~/ai-factory/engine/scripts/verify-session-resume.mjs codex` (add `cursor` when it is configured). The script's own verdicts were checked with fake providers that remember and that forget a session, and with a missing binary.
