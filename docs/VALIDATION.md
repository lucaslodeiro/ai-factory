...
- `npm test`: 431 tests, 431 passed. `dashboard serves readable state and queues daemon controls` had been failing on `main` since `a2d87a5` wherever no Codex CLI is installed, CI included: provider validation checks every selected role, the other roles stayed on Codex, and the test read Codex from its `.env` instead of its fake. The test now passes its fake Codex with the Cursor change.
