...
## Follow-up: conversation layout fixes

From a full-page review of the dashboard with issue #10 at 1280 px, 390 px and in dark mode (no console errors, no horizontal scroll):

- **Bodyless newest turn opened.** The newest turn opened by default even when it was a transition with nothing to show, leaving a stray divider and a clipped card. Now the newest turn *with a body* opens (the Reviewer report on issue #10), a transition without details never opens by default, and one opened by hand shows no divider.
- **Header notes on narrow screens.** "Continued here" and the status-comment note placed the label beside an eight-line column of text at 390 px; they now stack the text under the label.
- **Filter row on narrow screens.** The label and the turn count each wrapped onto two lines; the label now keeps one line and the count moves below it without wrapping.
- **Nested scroll.** The conversation scrolled inside a 38 rem box inside the page, cutting the last turn in half. It now grows with the page; the scroll-position bookkeeping that only served the inner scroll is gone.
- **Indistinguishable times.** Every issue #10 turn read "2h ago". Turn times stay relative within the last hour and become the clock time after that, with the date when it is not today.
- Browser check: desktop and 390 px showed no inner scroll, only "Reviewer · pass" open, clock times in the turn headers, the note stacked at 390 px (text 254 px wide inside a 282 px note), and the filter count on its own line. See [issue-10-conversation-open.png](2026-09-22-publication-visibility/issue-10-conversation-open.png) and [issue-10-mobile-notes.png](2026-09-22-publication-visibility/issue-10-mobile-notes.png).
- Tests: **turns open only when they have a body and show the clock time once they are older than an hour** (`test/dashboard-settings-feedback.test.ts`); the milestone filter test now expects the newest execution, not the bodyless transition, to open; the refresh test no longer checks the removed inner scroll. `npm test` — **392 pass, 0 fail, 0 skipped**.

## Follow-up: Recent events becomes System activity

- The global feed repeated each issue's conversation: transitions, agent results, execution starts and finishes. Things with no other home, such as GitHub publication and sync problems, maintenance, Slack delivery and failed controls, were buried among them.
- The feed now excludes workflow transitions and presentation changes, agent results, model selection, verification results, confirmed GitHub publications, and every `execution.*` and `command.*` event. Everything else appears, so a new system event type shows up by default instead of disappearing. The presentation branches and helpers that only served the excluded types were removed.
- The panel is titled **System activity**, says that workflow progress lives in each issue's conversation, and shows "No system activity. Workflow progress appears in each issue's conversation." when empty.
- Browser check on issue #10, rebuilt on `main` at `f4a3604`: the panel was empty with that sentence even though the item had a full conversation. After a stalled GitHub publication and a failed Slack delivery were recorded, it listed exactly those two, each linked to issue #10. No console errors. See [system-activity.png](2026-09-22-publication-visibility/system-activity.png).
- Test: `test/dashboard.test.ts` now asserts that no workflow, agent, execution, command or model event reaches the feed, while a seeded GitHub publication failure does, with its issue link. The assertion was shown failing without the backend filter.
