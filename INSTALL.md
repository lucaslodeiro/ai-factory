...
The **Configuration** panel has eight categories. **Credentials** reports authentication for GitHub, Claude and Codex. **Connect** or **Reconnect** starts that provider's official CLI login in the background and opens its browser flow on the Mac running the dashboard; complete the provider page and use **Refresh status** if the card has not updated yet. Credentials stay in each CLI's local secure store. The dashboard never receives or displays their tokens. Login output is available in `.factory/service-logs/credentials.log`.

The other seven categories edit the installation's `.env` without exposing secrets to the browser. They cover project and GitHub, runtime, dashboard, model routing, agent tools, access and secrets, and notifications. Closed values use selects: dashboard hosts are restricted to loopback addresses, while Codex and Claude model profiles expose their supported configured choices. An existing custom model ID is retained as the current selectable value. Every field shows its environment-variable name and which service must restart. A configured Slack webhook is shown only as present; leaving its field blank preserves it, while the explicit clear checkbox removes it. Values are validated with the same constraints as the terminal configurator. Each save creates a private `.env.backup-*` and atomically replaces `.env` with owner-only permissions. Unknown existing settings are preserved.
