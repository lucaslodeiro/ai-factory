...
For an interrupted update that already unloaded both services, run `AI_FACTORY_UPDATE_STATE_FILE="$PWD/.factory/update-state.json" ai-factory update --start-services` from the installation directory. This explicit recovery mode starts both services after a successful update. Normal dashboard updates persist each service's original loaded state before the background job begins and restore that recorded intent after success or failure.
