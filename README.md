...
FAILED issues receive a readable troubleshooting comment containing the stage, matching execution metadata, sanitized failure reason, a bounded `stderr` tail when available, and exact retry instructions. Full logs stay local in the dashboard. Guidance written above or below `/factory retry` is echoed in the acceptance comment and becomes an explicit human instruction for every remaining agent in that delivery; it takes priority over conflicting suggestions from earlier agent reports.
