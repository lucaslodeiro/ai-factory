...
| **Product Designer** | **Designer** | **Design** | Runs only when the Architect assesses a significant UX impact: builds a disposable prototype of the written specification with screenshots that the human approves before the Builder starts. It never reaches the pull request. | Claude |
...
The installer prepares the engine, installs both macOS services, starts the local dashboard and opens its first-time setup page. The same six Configuration groups are shown for setup and daily use, with required fields marked during setup. The daemon remains stopped while required setup is incomplete; the first valid **Save and apply** starts and verifies it automatically, and the setup layout ends automatically once every requirement is met. In an existing source checkout:
...
- `/factory approve vN [guidance]` approves brief vN, including its split into stories when there is one, so the Architect writes the SPEC; for a significant UX change it later approves the prototype of that SPEC.
...
The daemon polls new comments normally. **Refresh issue list** reconciles only already tracked issues, advances each to its newest comment and evaluates that comment for the current request; it never starts an unassigned issue or replays old work. Closed issues disappear from operational views. The editable GitHub status comment carries current stage, condition, a compact machine-readable state index and exactly one action; labels use independent stage and condition dimensions.

Each issue detail in the dashboard includes a live workflow conversation: the prompt metadata sent to every agent, agent results, orchestrator transitions and failures, and human interventions. Exact prompts stay collapsed behind the existing sensitive-content acknowledgement. An authenticated GitHub operator listed in `FACTORY_APPROVERS` can send state-specific answers, approvals, guidance and notes; every dashboard turn is published to the issue first under that GitHub identity and then applied locally, so GitHub remains the audit trail.
...
Product Architect records a complexity/risk assessment and a verification depth of minimal, standard or thorough, all approved with the brief before any specification is written, so each issue receives the testing it earns. It may also split an epic into two to four stories, approved with the same brief: each story becomes a sub-issue blocked by the stories it waits for, runs Builder and Tester on its own branch from the epic branch, and merges into it; the epic then goes through Test and Review once and opens the single pull request. Depth cannot be declared below the worse of complexity and risk. Findings carry a severity: only a critical or major finding returns work to the Implementation Engineer, while a minor finding is deferred and recorded without costing a correction cycle. Each issue has a budget of provider-reported tokens, 5,000,000 by default, spent by every run including retries and corrections; when it is reached, the next run waits until an approver posts `/factory budget +<tokens>`, and a run without reported usage waits for acknowledgement instead of counting as zero. See [Token budget per issue](INSTALL.md#token-budget-per-issue). Configuration → Agent roles chooses Codex, Claude or Cursor and one direct model for Product Architect, Implementation Engineer, Verification Engineer and Delivery Reviewer. Each model can also be **Auto**, which delegates model choice to its provider. See [model selection policy](docs/MODEL_POLICY.md).
...
GitHub issues show the workflow through colored stage/condition labels and an updatable status comment. Durable workflow facts required for cross-installation continuation are embedded in Factory-authored issue comments; execution logs, prompts, secrets and local paths stay local. The issue remains open until the delivered PR is merged.
