...
Use the balanced-v4 policy in `docs/MODEL_POLICY.md`: Product/Architect reports complexity, risk and rationale with each new spec; the human approves that assessment with the exact spec version. The orchestrator deterministically selects a profile by role, approved assessment and correction context, then uses that role's configured Codex/Claude provider. In manual mode it passes the configured profile model; in auto mode it omits the model override so the provider selects its recommended/default model. QA/Reviewer retain a balanced floor, high-risk/complex work and correction cycles select strong, and only low-risk/simple Developer work selects fast. A high-complexity or high-risk draft produced below the strong profile receives a fresh strong-profile Architect review before a version is published for approval; questions and retries preserve that review requirement. Every provider call records its configured selection. No silent provider or mode fallback. Provider and model mode/mappings remain operator-configurable per role.
